...
For every selected value, the helper counts exact raw-byte occurrences across every blob in the complete base tree and complete review snapshot, including ordinary text, comments, symlink targets, and binary content. A source-WIP review separately counts its committed source `HEAD`, because that tree remains readable from the helper-private Git database even when the WIP snapshot deletes or replaces its content. This counter is independent of scanner suppression events and continues even after another scanner rule blocks the surface. Clean reviews require `head_count <= base_count`; source-WIP reviews require both `head_count <= base_count` and `source_head_count <= base_count`, where `head_count` names the final review snapshot:
...
The helper also records an unembedded count: an exact occurrence is unembedded only when no strictly longer value in the same selected envelope completely contains it. Raw and unembedded counts must be monotonic for both the final snapshot and committed source `HEAD`. This prevents either the commit or its WIP overlay from deleting a longer registered value and reusing one of its registered substrings as a new standalone value, even in plain text or binary content.

The rule is global, not path-bound or blob-bound. It allows a historical fixture to move while preventing new use. Unknown IDs, duplicate selections, duplicate exact values, any authoring-related or cross-envelope value overlap, malformed entries, count failures, and an entirely unused selected envelope fail closed. Historical master can contain one legacy exact value inside another, so legacy-only substring relationships are allowed only inside one envelope; every descriptor is counted independently at every start position. Complete scanner-captured equality still selects only the declared exact descriptor. Prepare keeps a raw-free authoritative count copy in helper-private container state outside the reviewer workspace. The stateful runner requires the workspace manifest to match that copy, reloads the same fixed catalog, recomputes the materialized snapshot counts, and, for a source-WIP review with a selected exemption, independently recomputes the committed source-`HEAD` counts before egress. Raw values are never copied into workspace control files or helper-private state.
...
Selection never turns a legacy value into an authoring token. Legacy exemptions apply only to count-proven tree content: every exact catalog legacy value remains forbidden in endpoint commit messages, ordinary headers, decoded signature metadata, and nested `mergetag` metadata. Legacy exemptions also never apply to prompts, so any resulting prompt scanner finding blocks. Credential-like paths and unrelated secrets in the same file remain blocking.
