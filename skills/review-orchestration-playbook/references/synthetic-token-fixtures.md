...
The same classifier applies to changed base and head blobs and the complete frozen head. The frozen diff and rendered prompt are intentionally integrity-bound but are not rescanned after the tracked range passes the reduction gate. Prompt bytes are trusted reviewer input outside catalog admission and tracked-tree reduction counting; this does not weaken credential-path or complete-head checks.
...
The automatic reduction gate for an unregistered dynamic secret is separate from this catalog. It may pass only when complete-tree exact counting proves `head_raw_count < base_raw_count` and `head_unembedded_count <= base_unembedded_count`. Equality, additions, moves/renames, copies, net growth, substring extraction, incomplete counts, and unsafe or uncountable candidates fail closed. A complete provider-specific span inside a secret-key assignment does not erase the longer generic identity: the scanner proves the bounded logical right-hand side across quoting, escaping, type-aware last-in-first-out wrapper matching, expressions, comments, stream frontiers, and diff record sides, and deduplicates only an exact candidate. Missing, crossed, mismatched, extra, or externally unclosed wrappers block once the suffix is complete; a structurally valid partial wrapper remains incomplete until the next stream window can prove its suffix. Provider-backed exhaustive assignment discovery skips secondary RHS walking when no provider span exists, advances line context incrementally, and reserves bounded prefix-proof budget before scanning candidate-bearing RHS bytes. The candidate's exact raw value and canonical Base64 encoding are also forbidden in every frozen or materialized head path. A base-only path removed by the range may remain in the trusted raw diff, but it is excluded from changed-head path evidence. Once that gate passes, the trusted reviewer receives the frozen tracked diff and necessary tracked context in their original form.
...
The reviewer-visible synthetic manifest, changed-path digest, changed-blob finding, accepted-evidence, diff, and prompt artifacts are created as owner-only `0600` files independently of the caller's permissive umask and are bound by exact name, byte size, SHA-256 digest, and, for NUL-delimited files, record count to helper-private state that contains no catalog token literals. Changed-head paths are represented publicly only by ordered SHA-256 commitments; their raw byte records are held in one ephemeral helper-private `0600` file and verified lockstep against the public commitments. Changed-blob findings also use path digests. All public evidence strings, including path commitments and artifact digests, are checked against authoring, legacy raw/storage, and dynamic reduction values before publication. The state also binds the `.codex-review` directory identity, non-group/other-writable mode, stable metadata, and exact six-entry name-set digest. Stateful preflight rejects any extra regular file, nested directory, symlink, or FIFO, then consumes each fixed public artifact and the private raw-path file through no-follow, nonblocking, owner-matching regular-file descriptors, applies byte and record limits, verifies stable metadata, and requires matching order, digest, count, and EOF. It then removes the private raw-path file and raw-bearing dynamic-reduction manifest through owner-, mode-, and identity-checked no-follow descriptors opened on the review root and container before writing `preflight.json` or launching the reviewer. Failed validation and every later cleanup path retry both exact names, including partial preparation, explicit keep, fallback retention, workspace layout-validation failure during cleanup, a retained surrounding state container, an already-missing workspace, or workspace-tree removal error. Review-root symlink replacement and unbound forged containers fail closed without deleting outside targets, while an error unlinking one fixed file does not skip the other. Explicit cleanup can also scrub those fixed names after state/layout corruption, but only from the resolved marked state directory while the runner lock is free and the cleanup lock is held; it continues to report the corruption. Clearing a deleted-path finding, swapping a control file, changing a path commitment, or changing the manifest, diff, or prompt therefore fails closed.
