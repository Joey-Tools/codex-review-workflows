...
Selection never turns a legacy value into an authoring token. The value remains scanned outside the selected, count-proven tracked-tree context. The prompt is integrity-bound but is not a secret-egress filter under the trusted-reviewer decision. Credential-like paths that remain at head and unrelated tracked-tree findings outside an eligible reduction remain blocking.
