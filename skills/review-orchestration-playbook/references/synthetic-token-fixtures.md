...
The same classifier applies to changed base and head blobs, the complete frozen head, and the rendered prompt. The frozen diff is intentionally integrity-bound but not rescanned after the tracked range passes the reduction gate. Prompt acceptance remains narrower than tracked-tree acceptance: only exact authoring-catalog values may pass, while legacy exemptions and dynamic reductions never apply there and do not weaken credential-path checks.
