...
For each complete stdout/stderr path, the caller must create and supply its lexical parent as an already-canonical real directory. The parent must be outside the review worktree, exist, be a non-symlink directory, and equal its own strict resolution. Each argument must name a distinct direct leaf in that parent; the leaf must be absent and non-symlink before launch and remain protected by no-follow, exclusive publication. The guard must not infer authority from a symlink-resolved parent or follow a pre-existing leaf symlink, including a dangling one. Before launch it binds the validated parent identity to an open directory descriptor. Temporary creation, hard-link publication, cleanup, and rollback remain relative to that descriptor, and the lexical path must still name the same `(st_dev, st_ino)` before and after publication. A failure to remove a temporary hard-link name rolls back every leaf already published by that write before returning inconclusive. Parent `mtime`, `ctime`, and child-count churn are not identity checks.
