...
1. Prepare a lane-unique Git workspace at the same frozen `head_sha` used by the Codex lane only through the trusted bundle's `named_lane_guard materialize-worktree`, followed immediately by its `validate-worktree`, under the [Shared Frozen-Range Contract](review-lane-contracts.md#shared-frozen-range-contract). The materializer—not an ambient shell—binds the exact source marker/admin/worktree root, rejects suffix-DWIM sources, and creates an owner-private, lane-local, no-checkout local clone with source/target ancestor discovery fenced, empty private config/template/hooks roots, no copied source config or hooks, no hardlinked/alternate/promisor object dependency, no remote transport, and hooks, fsmonitor, system/global attributes/config, submodule recursion, executable clean/smudge/process filters, and external diff commands disabled before clone initialization or detached checkout. It disables commit-graph and multi-pack-index consumption, persists both Git-false for Claude's later bare Git, safely removes every copied pack/MIDX bitmap cache, and runs bounded full object-validity `fsck` over the frozen endpoints before rejecting shallow, partial, incomplete, unexpectedly configured, corrupt, or unsafe target repositories. Never use `git worktree add`, an ordinary checkout-producing clone, or a source/target `git status`, `diff-files`, or `diff-index` as lane preparation. `validate-worktree` then performs the first status query under the same cache and discovery fences, verifies exact `HEAD`, clean ordinary/staged/index state, safe gitlinks/symlinks/guidance, and direct config safety, and binds the same materialized path/full head. Remove all `remote.*` configuration before checkout and model launch. Record the trusted bundle's absolute source, version, and SHA-256 digest, never resolve a repository-relative guard, and do not launch unless both guard subcommands succeed back-to-back. A trusted prior bundle that lacks the new materializer may bootstrap this self-policy migration only under its prior policy; activate this step after the merged release. Every bounded materialization, validation, or cleanup failure is `blocked-safety`, with the exact retained path reported when cleanup cannot complete.
