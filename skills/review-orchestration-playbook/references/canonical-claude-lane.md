...
3. require exactly Claude Code `2.1.212` from the accepted preflight's snapshot-bound version evidence; if a later phase repeats a version or advertised-option/capability probe, first repeat publisher verification and run that non-review probe only from a new private digest-verified snapshot while the descriptor-bound source identity remains stable;
4. validate the advertised option/capability surface only after publisher verification and without exposing credentials, repository content, range, PR, or review-workspace input; and
