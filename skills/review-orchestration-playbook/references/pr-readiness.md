...
- A standalone single, double, or triple request may locate an already-existing PR read-only when the request names it or the current branch has one unambiguous PR, solely to derive/verify the intended range and enforce head alignment. This lookup does not authorize other PR-state consumption or mutation; only bare triple additionally authorizes the scoped GitHub Codex request comment described below.
...
1. Establish or locate the PR only when the parent request permits that operation. A standalone named single, double, or triple request may locate an already-existing PR under the narrow read-only authorization above, but must not create or mutate one. Only actual PR absence takes the no-PR path; for requested triple that path is effective double. An existing PR on an unsupported host or identity remains on the existing-PR path even though its third lane is unavailable. At this stage record only the stable repository/PR identity needed to address later probes. Do not yet read or consume review threads, required checks, rulesets, mergeability, or other readiness state; do not require PR-only fields on the no-PR path.
2. Preserve any parent-provided frozen `base_sha..head_sha` as the intended range before deriving it from PR state. Only when a PR/full-workflow request or standalone named review associated with an existing PR has no preexisting frozen range may the parent query the minimum base/head commit identity needed to derive the intended `<merge_base>..<head_sha>` from the current PR head. Keep that derived intended range independent from later current-head observations.
