...
4. After the helper preflight passes, run `independent-codex-pr-review` in a fresh Codex CLI review-only session. Capture its complete stdout and stderr in task-scoped files instead of streaming either process output into the parent thread, and pass `--output-last-message <task-scoped-file>` with a fresh path that did not exist before this attempt. Before launch, record a finite wall-clock deadline plus byte limits for each process log and the final-message artifact; use the default 30-minute / 16-MiB / 64-KiB limits from the shared contract unless a stricter repo-local contract applies. Launch the reviewer in a dedicated process group or session and enforce the process-log caps with hard file limits or bounded sinks. Poll only with bounded status probes and terminate the entire group when the deadline expires or either process-log limit is reached. A limit-terminated attempt is inconclusive, cannot contribute a final-message result, and retains only bounded diagnostic evidence. Wait for every group member to exit and close inherited sinks before you stat both process logs again; reject an attempt whose final log size reaches or exceeds its limit, even after exit zero. If group exit cannot be confirmed after the bounded kill wait, retain bounded evidence and report `inconclusive`. Only then accept the final-message file when this attempt exits zero and creates a nonempty file within its byte limit; check its byte count before reading it, and reject, classify, and remove an oversized artifact without reading it. Never reuse a stale or partial result. For a nonzero exit or a missing/empty final-message file, read one bounded stderr error tail to distinguish a deterministic blocker from an otherwise inconclusive run; never relay the complete process logs. Disable project-instruction injection, preserve the complete **Scope And Evidence Budget** and **Parent-Process Output Budget** contracts from [review-lane-contracts.md](review-lane-contracts.md), forbid PR orchestration/fixes/other reviewers/CI waiting, and require a terminal `LGTM` or no-findings artifact. GitHub Codex and the helper-backed frozen-diff lane cannot replace this gate.
