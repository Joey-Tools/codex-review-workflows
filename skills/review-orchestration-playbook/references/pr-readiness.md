...
Prefer a GitHub review whose `commit_id` equals the current `headRefOid`. For a same-head terminal or no-start response available only as an issue comment, [the issue-comment-only correlation rule](github-pr-probes.md#issue-comment-only-correlation) is authoritative: after the expected-author and head-time/stability checks, accept explicit correlation to the exact request/run or the full exact current SHA, or use its sole-unresolved-request fallback. An older request remains unresolved across a head change unless trustworthy correlated evidence resolves it; without explicit correlation, that ambiguity is `triple-inconclusive`.

Any push invalidates the old evidence. Request a new current-head review rather than reusing a stale comment.

Effective-double fallback requires directly known no-PR/host/identity evidence or a properly correlated authenticated provider result that proves the integration/service unavailable before any run starts. Posting the request comment is not service start. Missing response, timeout, comment-write or generic HTTP failure, or evidence that proves neither unavailable nor started is `requested: triple`, `effective: triple-inconclusive`. Once acknowledgement or run/review activity proves service start, malformed, stale, ambiguous, or transiently incomplete evidence is also triple-inconclusive and must not be converted to effective double or completed triple.
