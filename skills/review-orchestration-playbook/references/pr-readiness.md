...
3. Run `offline-frozen-diff-review` first through the stateful pinned Codex helper on the exact frozen range. Require its retained `preflight.json` before launching any separate Codex session; this proves the frozen workspace, diff, and prompt passed the sensitive-content and escaping-symlink checks. If the reviewer runtime becomes unavailable only after that preflight, the configured clean-context `reviewer` fallback with the same frozen scope and evidence contract may replace the review artifact. If the helper cannot complete the preflight itself, stop instead of bypassing it.
