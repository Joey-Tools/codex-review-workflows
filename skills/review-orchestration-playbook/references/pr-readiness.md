...
5. For a supported third lane, post the exact `@codex review` comment after `head_sha` becomes current. Record the comment URL/time. The comment write is not completion or proof of service start. An authenticated provider rejection may prove no-start integration/service unavailability only when it is tied to the exact request/dispatch or satisfies the sole-unresolved fallback; a full SHA alone binds the head, not a same-head request. Acknowledgement or run/review activity proves start. Accept only a terminal result bound to the same head. For same-head terminal or no-start evidence that exists only as an issue comment, defer to the request-ledger and correlation rule in [github-pr-probes.md](github-pr-probes.md#issue-comment-only-correlation); do not pair comments by nearest timestamp.
...
Prefer a GitHub review whose `commit_id` equals the current `headRefOid`. For same-head evidence available only as an issue comment, [the issue-comment-only correlation rule](github-pr-probes.md#issue-comment-only-correlation) is authoritative. A terminal completion may bind through the exact request/run, the full exact current SHA, or the sole-unresolved-request fallback. A no-start rejection must instead bind to the exact request/dispatch or satisfy the sole-unresolved/no-intervening fallback; its SHA alone is insufficient when another same-head request remains unresolved. An older request remains unresolved across a head change unless trustworthy correlated evidence resolves it; without the required correlation, that ambiguity is `triple-inconclusive`.

Any push invalidates the old evidence. Request a new current-head review rather than reusing a stale comment.

Effective-double fallback requires directly known no-PR/host/identity evidence or a properly request-bound authenticated provider result that proves the integration/service unavailable before any run starts. Exact current-head SHA binding alone is not request binding for no-start evidence. Posting the request comment is not service start. Missing response, timeout, comment-write or generic HTTP failure, or evidence that proves neither unavailable nor started is `requested: triple`, `effective: triple-inconclusive`. Once acknowledgement or run/review activity proves service start, malformed, stale, ambiguous, or transiently incomplete evidence is also triple-inconclusive and must not be converted to effective double or completed triple.
