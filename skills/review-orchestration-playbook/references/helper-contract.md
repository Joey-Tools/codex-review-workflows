...
The Claude-family lane requires one of `--egress-consent explicit-claude-review`, `--egress-consent double-review`, or `--egress-consent triple-review`. The helper saves this value in state and writes `egress.json`; it refuses to start the external lane without it. `explicit-claude-review` authorizes Anthropic only, while `double-review` and `triple-review` also authorize GitHub Copilot fallback when Claude Code is unavailable or both pinned Claude models are entitlement-blocked.
...
- Snapshot preparation runs Git with a cleaned environment, disabled hooks/fsmonitor/external diff, and explicit `--no-ext-diff --no-textconv`. It does not checkout files through repository filters, and it removes a partial review container on ordinary errors, forwarded signals, or `KeyboardInterrupt`. If removal fails before the state path is published, the terminal error names the retained container and cleanup failure instead of silently losing recovery evidence.
...
- The helper installs the fixed `git_readonly_shim`, passes the real Git path separately, and executes the reviewer inside the detached workspace. The shim requires the exact isolated root, sets its container as Git's discovery ceiling, rejects a cwd outside the root, and blocks repository-routing options such as `-C`, `--bare`, `--git-dir`, `--git-common-dir`, and `--work-tree`; it cannot discover or select the parent source repository. Diff-like commands also reject `--output`, so an otherwise read-only Git command cannot write a report file, and `symbolic-ref` is limited to one-ref queries rather than mutation forms.
...
- Codex runs with a custom permission profile: only platform-minimal paths, the frozen workspace, and the exact trusted Python runtime root required by the Git shim are readable; `.git`, `.codex`, `.agents`, and environment-file globs are denied; network and writes are unavailable. It ignores user config and execpolicy rules, sets `project_doc_max_bytes=0` so the reviewed head cannot inject `AGENTS.md` instructions, uses `approval_policy=never`, and gives model-proposed shell commands a fresh environment with an empty helper-owned `HOME`. The helper-owned review prompt is the only project-level review contract. The terminal artifact is rejected unless the persisted Codex turn context exactly matches the expected restricted filesystem entries, trusted shim runtime, optional helper-owned arg transport, denied control paths, restricted network, and never-approve policy; extra read paths/globs and legacy or managed `sandbox_mode` overrides fail closed as `permission-mismatch`.
- Claude Code runs in safe mode with `dontAsk` permissions and only `Read`, `Grep`, and `Glob`; no additional directory is allowed, so its permission root is the frozen workspace. Before every attempt, the helper requires current `claude --help` output to confirm that safe mode disables `CLAUDE.md` and all other project customizations while preserving auth, model selection, built-in tools, and permissions; otherwise the lane fails closed. Explicit deny rules cover common credential/config homes. Slash commands, Chrome integration, inherited MCP configuration, repository/user setting sources, and nonessential traffic are disabled.
- Copilot runs in plan mode with `-C` pinned to the frozen workspace, no `--add-dir` or `--allow-all-paths`, explicit shell/write/URL denial, temp-directory denial, and disabled custom instructions, built-in MCPs, bash environment loading, experimental features, and remote session export. Before every attempt, the helper requires `copilot help permissions` to confirm that default file access is cwd-only, `--disallow-temp-dir` removes the implicit temp root, and deny rules override `--allow-all-tools`; otherwise the fallback fails closed. Secret-like auth variables are withheld from its tools.
- Before any network-backed Codex, Claude Code, or Copilot run, the helper rejects any symlink in the frozen workspace that resolves outside that workspace and blocks credential-like paths or high-confidence secret patterns found across base-to-head changed paths, both sides of changed raw blobs, every head-snapshot path name and file, the frozen diff, or prompt. Changed paths and raw-blob findings stream to NUL-delimited control files and are scanned incrementally, including deleted binary credentials and credential filenames nested under fixtures or copied home directories. Secret-bearing snapshot path names are redacted from diagnostics; findings record only bounded side/path/rule metadata, never matched secret values.
...
- Claude prompt suggestions are disabled. If Claude still reports auxiliary model usage, effective-model verification selects the explicitly requested model entry rather than treating the first auxiliary entry as the reviewer model.
...
- Source files are never edited. The detached workspace is removed after `stateful wait` unless `--keep-workspace` is set. A finite wait deadline covers runner completion, cleanup-lock acquisition, and workspace removal; `stateful final` applies its own bounded cleanup deadline. Timed cleanup runs in an independent worker that inherits the held cleanup-lock descriptor, so a timeout can return without letting another waiter start a concurrent `rmtree`; a daemon reaper collects the worker while long-lived callers continue.
- Logs, attempts metadata, and `final.txt` remain in the state directory after workspace cleanup. `attempts.json` is atomically rewritten after every completed model attempt, so cancellation during a later entitlement fallback preserves the earlier model/category/effective-runtime evidence.
