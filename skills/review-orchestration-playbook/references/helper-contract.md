...
- Claude output is accepted only from the single documented top-level `result/success` object with `is_error=false`, non-empty `result`, and model usage metadata. Copilot JSONL output requires a matching `assistant.turn_start` / `assistant.turn_end` boundary and accepts only the last top-level `assistant.message` inside that turn when `toolRequests` is absent or empty; model verification uses the corresponding `assistant.usage` event when emitted, otherwise the current CLI's message model or `session.start.selectedModel` extension. Arbitrary progress, tool, nested `message`, `content`, or `data` fields never become a terminal artifact. Any structured error clears the result.
