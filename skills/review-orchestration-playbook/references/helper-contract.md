...
- Claude output is accepted only from the single documented top-level `result/success` object with `is_error=false`, non-empty `result`, and model usage metadata. Copilot JSONL output is accepted only from a non-empty, tool-free `assistant.message` whose `turnId` matches a later `assistant.turn_end`; arbitrary progress, tool, nested `message`, `content`, or `data` fields never become a terminal artifact. Any structured error clears the result.
