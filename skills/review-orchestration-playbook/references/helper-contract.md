...
- Claude output is accepted only when the entire output is the single documented top-level `result/success` JSON object with `is_error=false`, non-empty `result`, and model usage metadata; only JSON's ASCII whitespace is valid outside the object. Structured error results never supply final text, but their model usage is retained so silent substitution still blocks fallback. Copilot JSONL is split only on LF, treats only JSON's ASCII whitespace as blank, and requires every other record to be a JSON object plus one non-nested, non-interleaved matching `assistant.turn_start` / `assistant.turn_end` boundary. It accepts only the last top-level `assistant.message` inside that turn when `toolRequests` is absent or empty; model verification uses consistent top-level `assistant.usage` events after that terminal message when emitted, rejects conflicts with the message model, and otherwise uses the current CLI's message model or `session.start.selectedModel` extension. Non-JSON prefixes or suffixes, arbitrary progress, tool, nested `message`, `content`, or `data` fields never become a terminal artifact. Any structured error clears the result.
