...
- Exclude ignored files according to the source worktree's effective path semantics without letting source configuration execute during operational inspection. A bounded config-only `git config --includes` query runs under the operating-system account home and actual source worktree context, allowing system, global, common-local, and linked-worktree precedence only long enough to select the final `core.excludesFile` plus typed `core.ignoreCase` and `core.precomposeUnicode` values. Resolve a relative excludes path against the canonical source root, secure-read at most 1 MiB from a stable current-user- or root-owned non-writable regular file, and freeze its bytes separately from the bounded source `info/exclude` snapshot. Source `status`, `diff`, `ls-files`, and `ls-tree` still use only the helper-owned Git database with system, global, and source-local configuration suppressed; the private Git config receives only the two validated path-semantics booleans, and `status` receives only the absolute helper-owned frozen excludes path. This preserves tracked repository ignores, `info/exclude`, effective configured excludes (including `config.worktree` overrides), the default `$XDG_CONFIG_HOME/git/ignore` or `$HOME/.config/git/ignore`, case-insensitive matching, and macOS Unicode precomposition without enabling hooks, aliases, filters, or diff commands. The deliberate `core.filemode=true` hardening remains independent so source configuration cannot hide a real mode-only WIP change.
