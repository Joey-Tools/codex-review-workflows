...
- `stateful start` blocks forwarded termination signals until the runner PID is saved and the state directory is printed, so a launch cannot leave an undiscoverable background reviewer.
