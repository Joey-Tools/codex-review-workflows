...
- Claude output is accepted only from the single documented top-level `result/success` object with `is_error=false`, non-empty `result`, and model usage metadata; structured error results never supply final text, but their model usage is retained so silent substitution still blocks fallback. Copilot JSONL output requires a matching `assistant.turn_start` / `assistant.turn_end` boundary and accepts only the last top-level `assistant.message` inside that turn when `toolRequests` is absent or empty; model verification uses consistent top-level `assistant.usage` events after that terminal message when emitted, rejects conflicts with the message model, and otherwise uses the current CLI's message model or `session.start.selectedModel` extension. Arbitrary progress, tool, nested `message`, `content`, or `data` fields never become a terminal artifact. Any structured error clears the result.
