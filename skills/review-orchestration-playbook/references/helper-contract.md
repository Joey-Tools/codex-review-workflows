...
4. Copilot CLI `claude-sonnet-5`, `max`, only when Claude Code is absent or all Claude Code models are entitlement-blocked
...
- Claude Code runs in safe mode with `dontAsk` permissions and only explicitly pre-approved `Read`, `Grep`, and `Glob`; no additional directory is allowed, so its permission root is the frozen workspace. Before every attempt, the helper requires current `claude --help` output to confirm that safe mode disables `CLAUDE.md` and all other project customizations while preserving model selection, built-in tools, and permissions and setting `CLAUDE_CODE_SAFE_MODE=1`; otherwise the lane fails closed. Explicit deny rules cover common credential/config homes. Slash commands, Chrome integration, inherited MCP configuration, repository/user setting sources, and nonessential traffic are disabled.
