...
- The helper installs the fixed `git_readonly_shim`, passes the real Git path separately, and executes the reviewer inside the detached workspace. The shim requires the exact isolated root, sets its container as Git's discovery ceiling, rejects a cwd outside the root, and blocks repository-routing options such as `-C`, `--git-dir`, `--git-common-dir`, and `--work-tree`; it cannot discover or select the parent source repository. Diff-like commands also reject `--output`, so an otherwise read-only Git command cannot write a report file.
...
- Logged reviewer processes run in their own process groups. The helper forwards `SIGTERM`, `SIGINT`, `SIGHUP`, and `SIGQUIT`, reaps the reviewer, and terminates leftover descendants before restoring signal handlers; a stateful runner persists `128 + signal` as its terminal exit code.
...
- Source files are never edited. The detached workspace is removed after `stateful wait` unless `--keep-workspace` is set. A finite wait deadline covers runner completion, cleanup-lock acquisition, and workspace removal; `stateful final` applies its own bounded cleanup deadline.
