...
- `stateful start` remains cancellable during workspace preparation. It defers a termination signal only across the narrow `Popen` handoff, checks it before publishing, and stops plus cleans any unpublished runner; after the runner PID is saved, it prints the state directory immediately so later cancellation still leaves discoverable terminal evidence.
