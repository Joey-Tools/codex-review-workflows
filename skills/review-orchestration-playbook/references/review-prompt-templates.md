...
Before applying the generic same-head/different-base `scope-mismatch` branch, if the audited request-time merge base changes after the accepted request while the head stays unchanged, invalidate the old whole-PR artifacts but do not post a replacement same-head request. Preserve any caller-supplied range and start no affected lane from it. A later local rereview requires an explicitly supplied current range, or normal rederivation when the invalidated range was parent-derived; it does not unblock the GitHub lane. Report readiness `blocked-input` (`base-changed-same-head`) and `requested: triple`, `effective: triple-inconclusive`; do not create an empty or anchor commit to manufacture a new head epoch.
