...
- Before constructing either local prompt, require parent-owned evidence that the trusted guard's `materialize-worktree` created the lane-private no-checkout clone under its isolated config/hooks/filter boundary and that the immediately following `validate-worktree` performed the first status query against the same full head/path. Never use `git worktree add`, an ordinary checkout-producing clone, a copied source config/hooks directory, or any pre-validator status query. If the trusted bundle lacks the materializer during its own policy migration, record the prior-policy bootstrap and do not claim the new boundary active before the merged release.
