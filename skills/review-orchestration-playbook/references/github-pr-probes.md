...
Before applying the generic same-head/different-base `scope-mismatch` branch, compare an accepted same-head request's audited request-time merge base with current `pr_merge_base`. If it changed while `headRefOid` remained unchanged, the old request/result no longer covers the whole PR and the same-head request limit prevents a replacement. Invalidate every whole-PR artifact, preserve a caller-supplied old range, start or count no affected lane from it, and report readiness `blocked-input` (`base-changed-same-head`) plus `requested: triple`, `effective: triple-inconclusive`. A later pass may run local lanes only from an explicitly supplied current range, or from a normally rederived range when the invalidated range was parent-derived; neither permits another same-head request. Eligibility returns only after a separately authorized ordinary change produces a new head, and no empty or anchor commit may manufacture that epoch.
