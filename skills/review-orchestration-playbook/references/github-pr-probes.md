...
gh api --hostname <host> --method GET --paginate --slurp \
  ...
  --jq '[.[][] | {id,api_url:("https://api.github.com/repos/<owner>/<repo>/reactions/" + (.id | tostring)),user:{login:.user.login,type:.user.type},content,created_at}]'
...
The reactions list does not supply a stable reaction-resource URL. For the only
supported third-lane host, exact `github.com`, construct `api_url` exactly as
`https://api.github.com/repos/<owner>/<repo>/reactions/<positive_decimal_id>`
from the already validated exact repository owner/name and the reaction's
canonical positive numeric `id`. Do not accept a caller-supplied URL, a raw
aggregate count, a noncanonical ID, or another API host. Reconstruct and compare
the same URL during the final re-read.
...
line, and server timestamps defined by the authority. Parse the authenticated
TLS response's `Date` header as the history `as_of_server_time`; record this
exact API URL as `as_of_api_url` and derive the fixed 2,592,000-second interval.
The local clock, a caller-supplied declaration object, or a bare
`window_days: 30` label is not evidence.
...
actor/state/raw and normalized body/native commit plus the complete associated
inline-comment pagination and thread join. Issue-comment snapshots include
exact actor/App/raw and normalized body/edit-aware server time and parsed
commit binding. An ID/time/commit summary alone is not acceptance evidence.

Immediately before success, repeat the lifecycle, base/head, unique merge-base, complete evidence, pagination, and selected-artifact reads. Require the exact whole-PR scope and the recorded `evidence_basis`—source channel, stable ID/URL, server time, and commit binding—to remain unchanged. For `thumbs-up-clean`, also re-fetch the authoritative provider declaration source/version/text, recompute its recorded normalization digest, and revalidate every ordered historical `samples[]` request/reaction record plus the current record field by field.
