...
If `pr_merge_base` changes after the accepted request while `headRefOid` remains unchanged, the old request/result no longer covers the whole PR and the same-head request limit prevents a replacement. Invalidate every whole-PR artifact, rerun affected local lanes only over a newly frozen range, and report readiness `blocked-input` (`base-changed-same-head`) plus `requested: triple`, `effective: triple-inconclusive`. Eligibility returns only after a separately authorized ordinary change produces a new head; never create an empty or anchor commit solely to manufacture that epoch.
