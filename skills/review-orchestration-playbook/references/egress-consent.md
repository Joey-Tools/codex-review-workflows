...
## Stateful Reviewer And Admission Evidence

When the low-level helper supplies PR/master evidence, collect the reviewer artifact with `stateful final --state-dir <state_dir>` and independently evaluate secret admission with `stateful admission --state-dir <state_dir>` on the same current-head state. Reviewer final may succeed while admission is violating or inconclusive. Only admission exit `0` (`clean`) permits admission; exit `1` means violations, exit `3` pending, and exit `75` inconclusive. Foreground review is not admission evidence, and a head change invalidates both terminal results.
