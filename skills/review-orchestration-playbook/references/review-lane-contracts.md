...
- Record the exact request comment's API ID and server `created_at`; require every accepted review/comment/check artifact to have its own trusted server timestamp strictly later than that request. Same-head evidence from an earlier request is stale. A qualifying check must have `status == "completed"`, `conclusion == "success"`, and non-null `completed_at` later than the request.
