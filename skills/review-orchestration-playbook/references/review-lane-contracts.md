...
- The existing supplied-diff/private-minimal-Git Codex helper is a different low-level mechanism and cannot satisfy this lane.
...
- Launch `<resolved-publisher-verified-claude-path>` directly from that worktree under [canonical-claude-lane.md](canonical-claude-lane.md). The supplied-diff/private-Git `isolated_review` helper is not the launcher for this lane.
...
- Treat the selected supported Claude Code release's `system/init` and capability output as evidence for only the fields it reports. It cannot attest the final merged sandbox, merged managed permission arrays, or actual path-rule evaluation; record the sandbox controls as requested configuration, not independently verified effective enforcement.
...
- Apply a version-profiled closed and session-bound contract to every intermediate event. The extended profile accepts only the reviewed `system/thinking_tokens`, assistant-message, user-tool-result, and `rate_limit_event` shapes; unknown intermediate type, subtype, field, session binding, or nested shape is inconclusive. Extended-2x success requires exact `fast_mode_state: off`, exact `terminal_reason: completed`, and nonnegative integer `time_to_request_ms`, `ttft_ms`, and `ttft_stream_ms`. Legacy success forbids those fields; extended failure permits them only when present with the same strict types and known values. Unknown terminal fields remain inconclusive.
- Capture bounded raw stdout in parent-owned state outside the model-visible worktree and pass it through [`validate_claude_stream.py`](../scripts/validate_claude_stream.py) with required `--claude-code-version <preflight-version>`, `--authentication-source <api-key|oauth-token|local-login>`, and `--process-returncode <child-returncode>`, plus the exact resolved cwd and requested model. It maps `api-key` to init `ANTHROPIC_API_KEY` and both `oauth-token` and `local-login` to init `none`. Only `classification: accepted` with a zero child return code supplies findings; a missing, invalid, or nonzero child return code is inconclusive when stdout contains an otherwise valid-looking success result. A nonzero code preserves deterministic structured `blocked` or `blocked-authentication` evidence, while a bare exit code creates neither. Prose inspection, an ad hoc parser, partial output, or any fail-closed validator result cannot satisfy the lane. Validator acceptance does not attest the merged sandbox.
...
- Before prompt, credential, repository, range, PR, or review-workspace input reaches Claude, require [`named_claude_preflight`](../scripts/named_claude_preflight) to select a strict stable publisher-verified release in `>=2.1.211,<3.0.0`. Its ordered candidates are an authoritative explicit absolute override followed by controlled active-install paths; caller `PATH` is ignored. Candidate presence is tri-state: exact absence may advance, but I/O, resolution, or identity uncertainty stops priority fallback as `candidate-inspection-inconclusive`. Before either probe, require supported native format plus the selected release's signed per-version manifest, exact platform size/SHA, and descriptor-bound strong source identity, including `ctime`. While that identity remains stable, materialize one private digest-verified executable snapshot and run mandatory credential-free `--version` and `--help` capability probes against that same snapshot with empty stdin, fixed `/` cwd, bounded time/output, and a fixed credential-free environment, never against the mutable installation path. Reverify the snapshot after both probes, then perform a fresh descriptor-bound hash of the mutable source against the signed size and SHA-256 before acceptance and recheck its identity; stat identity alone is not sufficient. Source-identity or digest drift is inconclusive and takes precedence over an observed wrong version. The helper emits one bounded JSON object, never downloads or installs a Claude executable, never changes an active symlink, and never executes scripts/wrappers, mutable install paths, or caller-`PATH` candidates. Deterministically invalid provenance is blocked as `publisher-verification-failed`; uncertain inspection stops as `candidate-inspection-inconclusive`; deterministic capability drift is blocked. Only accepted selection may proceed to canonical provenance revalidation and direct launch.
- Acquiring a missing supported release is a separate host mutation requiring explicit authorization and the official installer/version manager. A named review request does not authorize installation, active-version switching, or symlink repair; without a supported installed release that passes preflight, the lane remains blocked.
- Apply **Canonical Executable Provenance** from [canonical-claude-lane.md](canonical-claude-lane.md) before review input is exposed. Use explicit-auth precedence `ANTHROPIC_API_KEY` > `CLAUDE_CODE_OAUTH_TOKEN` > ordinary local login, opaque-forward only the winning explicit value, and require the publisher-verified CLI's `auth status --json` to agree with the selected source. Both this direct lane and the low-level helper use ordinary Claude CLI authentication in real `HOME`; the trusted CLI may update ordinary CLI-owned authentication and runtime state, including credential refresh and possible cache or tool-result artifacts. Those side effects are not model-authorized review mutations.
- Select the closed init profile from the accepted version: the legacy base profile covers `>=2.1.211,<2.1.216`, while the extended 2.x profile covers `>=2.1.216,<3.0.0` and requires exact additional fields. Added, missing, malformed, reordered, or fixed-value-drifted profile evidence is inconclusive.
...
- Supported-version, provenance, capability, or authentication failure does not change a requested double into single: the result is double-but-blocked or inconclusive. It also leaves a requested triple incomplete. Independently proved GitHub Codex unavailability may make the effective shape double, but that effective double is incomplete while its Claude lane is blocked or inconclusive.
