...
- Before prompt, credential, repository, range, PR, or review-workspace input reaches Claude, require [`named_claude_preflight`](../scripts/named_claude_preflight) to select exact `2.1.212`. Its ordered candidates are an explicit absolute override (authoritative and never followed by fallback on error), side-by-side `$HOME/.local/share/claude/versions/2.1.212`, then controlled active-install paths `$HOME/.local/bin/claude`, `/opt/homebrew/bin/claude`, and `/usr/local/bin/claude`; caller `PATH` is ignored. A declared non-`2.1.212` installer target is blocked without execution. Before executing any other candidate, require supported native format plus exact signed-`2.1.212` manifest size/SHA verification; only the verified artifact receives `--version` with empty stdin, fixed `/` cwd, bounded time/output, and a fixed credential-free environment. The helper emits one bounded JSON object, never downloads/installs a Claude executable or changes an active symlink, never executes scripts/wrappers or caller-`PATH` candidates, and returns a fixed absolute path with non-secret signed-artifact/identity metadata only on success. Missing/unusable exact version is blocked as `exact-version-unavailable`; any declared, artifact, or probed wrong version, including `2.1.216`, is blocked as `exact-version-mismatch`. A transient native-format or filesystem-identity inspection failure is `candidate-inspection-inconclusive`; publisher-verification, identity, and probe uncertainty are likewise inconclusive and must not be collapsed into deterministic unavailability. Only accepted selection may proceed to canonical provenance revalidation, capability verification, and direct launch.
...
- Record the exact request comment's API ID and server `created_at`. Server timestamps prove ordering, not request/run lineage. Review/comment APIs expose no request/run identifier, so a review/comment is `triple-inconclusive` whenever an older request might overlap, even if its trusted server `submitted_at` or `created_at` is strictly later than the current request. Subject to proved request isolation, a qualifying check must have `status == "completed"`, `conclusion == "success"`, and both non-null `started_at` and `completed_at` strictly later than the current request's `created_at`. Same-head evidence from an earlier request is stale.
