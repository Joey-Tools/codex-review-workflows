...
- Launch `claude` directly from that worktree under [canonical-claude-lane.md](canonical-claude-lane.md). The supplied-diff/private-minimal-Git `isolated_review` helper is not the launcher for this lane.
...
- Apply **Canonical Executable Provenance** from [canonical-claude-lane.md](canonical-claude-lane.md). [claude-runtime-trust.md](claude-runtime-trust.md) supplies shared signed-manifest verification primitives, version bounds, failure vocabulary, and the separate low-level helper contract only; the helper's verified executable snapshot, supplied-diff/private-minimal-Git workspace, explicit-auth precedence, clean/WIP capture, and post-run validation do not apply to this direct lane.
...
## Low-Level Helper Non-Lane Contract

`isolated_review` remains a diagnostic-only compatibility mechanism. Its state records `review_contract=supplied-diff-private-git` and `named_lane_eligible=false`, so neither its Codex path nor its Claude path can complete a named lane.

- The helper defaults to a clean exact-head detached workspace backed by helper-private minimal Git and a supplied diff. An explicitly authorized `--include-source-wip` run may instead capture staged, unstaged, and non-ignored untracked content into a digest-bound review-only snapshot; that artifact never satisfies a formal named lane or PR-readiness gate.
- Its Claude path preserves real `HOME` as the ordinary CLI control plane and selects `ANTHROPIC_API_KEY` > `CLAUDE_CODE_OAUTH_TOKEN` > local login. Only the winning explicit value is opaque-forwarded, and it is not persisted in helper state.
- Its native sandbox is selected-deny defense in depth, not a global host-read whitelist. Requested global `denyWrite` and critical-sensitive-root `denyRead` controls define the enforcement boundary; all other outside-workspace reads remain forbidden by prompt/model scope, and Claude Code 2.1.212 init output does not prove the merged sandbox.
- Helper egress markers are limited to `explicit-claude-review` and `explicit-claude-with-copilot-fallback`. The latter requires separate explicit Copilot consent, and the resulting diagnostic still does not increment a named review shape.

See [helper-contract.md](helper-contract.md) and [claude-runtime-trust.md](claude-runtime-trust.md) for the complete helper-only contract.
