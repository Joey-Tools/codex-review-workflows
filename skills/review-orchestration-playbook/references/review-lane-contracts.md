...
- Before launch, record a finite wall-clock deadline, a byte limit for each process log, and a byte limit for the final-message artifact. Unless a stricter repo-local contract applies, use a 30-minute deadline, 16 MiB per stdout/stderr file, and 64 KiB for the final-message file. Launch the reviewer in a dedicated process group or session and enforce each process-log cap at write time with a hard per-file limit or a bounded sink; polling alone is not enforcement. During bounded polling, when the deadline expires or either process-log limit is reached, send `TERM` to the entire group, allow one bounded grace interval, then send `KILL` to the group if any member remains. Wait for the whole group to exit and close every inherited log sink before the final checks. If group exit cannot be confirmed after the bounded kill wait, retain the bounded evidence and report `inconclusive` instead of accepting a result or cleaning files that may still be open. Do not accept a final-message artifact from a limit-terminated attempt.
