...
- Launch `<resolved-compatible-claude-path>` directly from that worktree under [canonical-claude-lane.md](canonical-claude-lane.md). The supplied-diff/private-Git `isolated_review` helper is not the launcher for this lane.
- Do not give Claude the Codex artifact, parent reasoning, or suspected findings.
- Use the detached worktree as review scope and real `HOME` as the trusted Claude CLI control plane. Select authentication with precedence `ANTHROPIC_API_KEY` > `CLAUDE_CODE_OAUTH_TOKEN` > ordinary local login, opaque-forward only the winning explicit value, and remove the lower-priority explicit source. The model may have `Read`, `Grep`, `Glob`, and sandboxed `Bash`.
...
- Map the selected source to observable init evidence exactly: `api-key` requires `apiKeySource: ANTHROPIC_API_KEY`, while `oauth-token` and `local-login` require `apiKeySource: none`. The init mapping distinguishes API-key from non-API-key operation only; bind the exact OAuth-token versus local-login selection with the earlier exact-CLI `auth status --json` evidence and the launch environment, without inspecting, logging, or persisting a credential value.
