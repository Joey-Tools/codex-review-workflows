...
- Before applying the generic same-head/different-base `scope-mismatch` branch, compare any accepted same-head request's audited request-time merge base with current `pr_merge_base`. A difference takes the dedicated `base-changed-same-head` branch: preserve a caller-supplied range and start or count no affected lane from it. A later pass may rerun local lanes only with an explicitly supplied current range, or with a normally rederived current range when the invalidated range was parent-derived. Neither local path unblocks the GitHub lane while `pr_head_oid` remains unchanged. Never post a replacement request or create an empty or anchor commit merely to regain eligibility; report readiness `blocked-input` and `requested: triple`, `effective: triple-inconclusive`.
