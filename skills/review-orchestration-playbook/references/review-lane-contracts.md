...
- Before launch, require `git status --porcelain` to be empty after the exact independently verified absent-gitlink exception below, `HEAD` to equal `head_sha`, both frozen commits to resolve, and read-only `git diff base_sha..head_sha` queries to work.
