...
- Before launch, record a finite wall-clock deadline, a byte limit for each process log, and a byte limit for the final-message artifact. Unless a stricter repo-local contract applies, use a 30-minute deadline, 16 MiB per stdout/stderr file, and 64 KiB for the final-message file. Enforce each process-log cap at write time with a hard per-file limit or a bounded sink; polling alone is not enforcement. Contain the reviewer with an OS-enforced job, cgroup, or container whose membership survives `setsid` / `setpgid`. If that is unavailable, launch in a dedicated process group or session and continuously discover every descendant, recording both PID and stable process-start identity before reparenting; signal those tracked descendants as well as the original group, repeat discovery and termination until no tracked identity remains, and never treat a bare PGID as the complete process tree. During bounded polling, when the deadline expires or either process-log limit is reached, send `TERM` to the containment unit and all tracked descendants, allow one bounded grace interval, then send `KILL` if any member remains. Require containment/tree quiescence and give inherited log sinks a separate 10-second close deadline before the final checks. If quiescence or sink closure cannot be confirmed, retain the bounded evidence and report `inconclusive` instead of accepting a result, waiting indefinitely, or cleaning files that may still be open. Do not accept a final-message artifact from a limit-terminated attempt.
...
- After the containment unit is empty or every tracked descendant identity is absent and all sinks close within their deadline, stat both process logs again before accepting any result. If either final size reaches or exceeds its recorded limit, reject the attempt as `inconclusive` even when it exited zero; record only the byte counts and remove the oversized log. Only then accept the separate final-message file when the attempt exits zero and creates it as a nonempty file within the recorded artifact byte limit. Check its byte count before reading it. If it exceeds the limit, do not read it, classify the attempt as `inconclusive`, record only its byte count, and remove the artifact. On a nonzero exit or a missing/empty file, reject any stale or partial result and read at most the final 8 KiB of stderr with a byte-count-limited read that truncates before inserting text into the parent transcript. A line-count-only command such as `tail -n` is not bounded when stderr contains a single long JSON or trace line. Use that bounded tail to classify an explicit authentication, permission, configuration, or runtime-verification failure as `blocked`; otherwise report `inconclusive`. Never read the complete stderr or reconstruct a clean result from stdout. Retained stdout and stderr are recovery evidence, not review findings.
