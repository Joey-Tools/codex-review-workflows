...
class ProbeCommandTest(unittest.TestCase):
    def test_builds_no_network_direct_executable_probe(self) -> None:
        ...
        with tempfile.TemporaryDirectory(
            ...
            self.assertIn(
                ("--ro-bind", str(home.resolve()), "/home/reviewer"),
                triples,
            )
            self.assertIn(
                (
            ...
            self.assertNotIn("socat", " ".join(command))
            self.assertNotIn("ripgrep", " ".join(command))
            self.assertEqual(
                command[-3:],
