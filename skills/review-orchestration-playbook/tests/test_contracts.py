...
_LFS_OID_RE = re.compile(rb"sha256:[0-9a-f]{64}\Z")
_LFS_EXTENSION_PREFIX_RE = re.compile(rb"ext-\d{1}-\w+")
_LFS_SIZE_RE = re.compile(rb"[+-]?[0-9]+\Z")
...
def _git_lfs_3_7_1_reference_pointer_gate(payload: bytes) -> bool:
    ...
    if not re.search(rb"git-media|hawser|git-lfs", data):
        return False
...
def _unique_parent_path_accounting(
    ...
    *,
    ...
    for raw_path in raw_paths:
        consumed_paths += 1
        if projection_inputs is not None and consumed_paths > projection_inputs.get(
            "entry_count"
        ):
            return {
        ...
        if raw_path_length == 0 or raw_path_length > max_path_bytes or max_depth < 1:
            return {
...
class RepositoryContractTest(unittest.TestCase):
    ...
    def test_workspace_defaults_clean_and_wip_is_explicit_review_only(self) -> None:
        ...
        self.assertIn("`HEAD`-to-snapshot delta paths", helper)
        self.assertIn("original-`HEAD`-side raw blobs", helper)
        self.assertIn("complete snapshot scan covers the current side", helper)
    ...
    def test_independent_codex_process_is_ephemeral_lightweight_and_bounded(
        ...
        self.assertIn(
            "retention root, checkout root, and common Git directory", readiness
        )
        self.assertIn(
            "group their unallocated physical headroom by filesystem identity",
            readiness,
        )
        self.assertIn(
            "combining process, checkout-root, and Git-admin remainders", readiness
        )
        self.assertIn(
            "different retention-root filesystem receives its own floor check",
            readiness,
        )
        ...
        self.assertIn(
            "ordinary placeholders for both regular and symlink leaves", readiness
        )
        ...
        self.assertIn(
            "exec acknowledgement and authenticated ready with no side effects",
            readiness,
        )
        ...
        self.assertIn(
            "`handoff=complete` plus `process_owner=attempt-supervisor`", readiness
        )
        self.assertIn("closes only its lock reference", readiness)
        self.assertIn(
            "retains the sole liveness/control peer plus supervisor handle", readiness
        )
        ...
        self.assertIn(
            "missing or invalid authorization forces `inconclusive`", readiness
        )
        ...
        self.assertIn(
            "Failure or uncertainty before complete retains the full envelope",
            readiness,
        )
        ...
        self.assertIn(
            "Controlled reader cancellation on a proven-no-child path", readiness
        )
        ...
        self.assertIn(
            "Never present requested values as observed effective values", readiness
        )
        ...
        self.assertIn(
            "Delete diagnostic logs after a clean no-findings result only", readiness
        )
        ...
        self.assertIn(
            "generation/digest-chained authoritative attempt-state record", readiness
        )
        ...
        self.assertIn(
            "different same-boot owner retains the full 257 MiB envelope", readiness
        )
        ...
        self.assertIn(
            "publication lease continues excluding successor owners", readiness
        )
        self.assertIn("non-TTL `retained-worktree` record", readiness)
        self.assertIn("starts its 7-day TTL at `released_at`", readiness)
        self.assertIn(
            "Pending-handoff and unresolved evidence has no age-based eviction",
            readiness,
        )
        ...
        self.assertIn(
            "exact detached-worktree path and allocated bytes when retained", readiness
        )
        ...
        self.assertIn(
            "create no task directory or prompt path before admission", contracts
        )
        ...
        self.assertIn(
            "neither mutates the ledger nor inherits the retention-lock descriptor",
            contracts,
        )
        self.assertIn("never in the outer process", contracts)
        self.assertIn(
            "starts exactly one phase helper with bounded record bytes", contracts
        )
        ...
        self.assertIn(
            "target filesystem's case-folding and Unicode-normalization behavior",
            contracts,
        )
        ...
        self.assertIn(
            "ordinary placeholder for every regular-file and symlink leaf", contracts
        )
        ...
        self.assertIn(
            "does not include the already-complete ownership handoff or checkout",
            contracts,
        )
        self.assertIn(
        ...
        self.assertIn(
            "supervisor starts the bounded native reader through the PID-first trampoline",
            contracts,
        )
        self.assertIn(
            "authenticated post-exec ready record before its first side effect",
            contracts,
        )
        self.assertIn(
            "one error-only close-on-exec acknowledgement descriptor", contracts
        )
        ...
        self.assertIn(
            "failure to commit `launched` before the reviewer-launch deadline",
            contracts,
        )
        ...
        self.assertIn(
            "successful, failed, timed-out, abandoned, or uncertain reviewer-launch cleanup path",
            contracts,
        )
        ...
        self.assertIn(
            "bounded native reader through the PID-first trampoline", contracts
        )
        self.assertIn("bind fd 0 to `/dev/null`", contracts)
        ...
        self.assertIn(
            "supervisor keeps the sole peer open until the reader is reaped", contracts
        )
        ...
        self.assertIn(
            "direct ordinary-file target is not a valid final transport", contracts
        )
        ...
        self.assertIn(
            "does not invalidate an otherwise complete final artifact", contracts
        )
        ...
        self.assertIn(
            "Observe an ordinary leader terminal state without reaping it", contracts
        )
        self.assertIn("one bounded post-signal drain", contracts)
        self.assertIn(
            "Reap the leader only after those signal and drain phases", contracts
        )
        ...
        self.assertIn(
            "Remove logs after a clean no-findings result only when", contracts
        )
        ...
        self.assertIn(
            "reclaim only eligible safely closed evidence oldest first", contracts
        )
        ...
        self.assertIn(
            "one authoritative `phase=reserved` attempt-state record", contracts
        )
        ...
        self.assertIn(
            "`reserved` proves that no worktree operation or reviewer began", contracts
        )
        self.assertIn(
        ...
        self.assertIn(
            "Once the supervisor's native primitive returns the Codex direct-child handle",
            contracts,
        )
        self.assertIn("After successful exec acknowledgement", contracts)
        self.assertIn("commits PID/PGID and `launched`", contracts)
        self.assertIn(
            "If that `launched` commit is interrupted or unverifiable", contracts
        )
        ...
        self.assertIn(
            "Recovery honors a complete evidence-bearing owner tuple as terminal",
            contracts,
        )
        ...
        self.assertIn(
            "closes the current owner's retention-root lock reference in a `finally` path",
            contracts,
        )
        ...
        self.assertIn(
            "Once the authoritative reservation state is committed", contracts
        )
        self.assertIn(
            "publication lease continues to exclude successor owners", contracts
        )
        ...
        self.assertIn(
            "only after every helper and subprocess from that phase", contracts
        )
        self.assertIn("non-TTL `retained-worktree` record", contracts)
        self.assertIn(
            "settle its 257 MiB reservation to exact retained process bytes", contracts
        )
        ...
        self.assertIn(
            "`prelaunch-aborted` never reports `clean` or `findings`", contracts
        )
        self.assertIn(
            "Report `admission_status`, `launch_status`, `review_status`", contracts
        )
        self.assertIn(
            "Recovery is exhaustive across every durable nonterminal phase", contracts
        )
        self.assertIn(
            "Every exact settlement is one complete authoritative tuple", contracts
        )
        self.assertIn(
            "`process_settlement=exact` may coexist with `checkout_settlement=outstanding`",
            contracts,
        )
        self.assertIn(
            "Exact-settled process evidence enters `held`, not `released`", contracts
        )
        ...
        self.assertIn(
            "`active/unsafe` -> `held` -> `released` -> `reclaimed`", contracts
        )
        self.assertIn("terminal status and age never release it", contracts)
        self.assertIn("`handoff-complete`", contracts)
        self.assertIn(
            "a requested, reported, or pending handoff remains held", contracts
        )
        self.assertIn("7-day TTL starts at `released_at`", contracts)
        self.assertIn(
            "Never reclaim active/unsafe or held/unresolved evidence at any age",
            contracts,
        )
        ...
        self.assertIn(
            "does not release evidence or move the attempt out of `active/unsafe`",
            contracts,
        )
        ...
        self.assertIn(
            "raw-object materialization intentionally need not satisfy", contracts
        )
        self.assertIn("re-prove both exact names", contracts)
        self.assertIn("`retained-worktree`", contracts)
        self.assertIn(
            "allocated-byte count to the terminal disk-footprint report", contracts
        )
    ...
    def test_checkout_charge_release_requires_durable_namespace_barriers(
        ...
        for name, content in candidates.items():
            with self.subTest(contract=name):
                ...
                directory_fsyncs = content.index("both already-open parent", removal)
    ...
    def test_checkout_cleanup_recovery_covers_all_authenticated_states(
        ...
        for name, content in candidates.items():
            with self.subTest(contract=name):
                cleanup = content.index("Before destructive checkout cleanup, require")
                ...
                no_more_deletion = content.index("no further unlink or rename", manual)
                ...
                settlement = content.index("`checkout_settlement=exact`", parent_fsyncs)
    ...
    def test_targeted_manifest_is_external_and_reserved_before_use(self) -> None:
        ...
        for name, content in candidates.items():
            with self.subTest(contract=name):
                ...
                settlement = content.index("`checkout_settlement=exact`", cleanup)
    ...
    def test_targeted_manifest_accounting_model_covers_all_ledgers(self) -> None:
        ...
        git_order_edge = _unique_parent_path_accounting((b"a.c", b"a/b", b"a0"))
        ...
        parent_accounting = _unique_parent_path_accounting(
            f"parent-{index:05d}/leaf".encode("ascii") for index in range(entry_count)
        )
        ...
        checkout_entry_bound = 1 + entry_count + unique_parent_directory_count + 3
        ...
        aligned_payload = ((payload_bound + a_checkout - 1) // a_checkout) * a_checkout
        file_bound = aligned_payload + a_checkout
        targeted_bound = 2 * file_bound + 2 * a_checkout
        checkout_parent_allocation_bound = a_checkout * unique_parent_directory_count
        checkout_root_bound = (
            checkout_base_bound_without_parents
            + checkout_parent_allocation_bound
            + targeted_bound
        ...
        baseline_checkout_failure = _unique_parent_path_accounting(
            ...
            projection_inputs={
                **projector_fixed_inputs,
                "checkout_cap": baseline_projection["checkout_accounting_bound"] - 1,
            },
        )
        self.assertEqual(
            "blocked-worktree-capacity",
        ...
        checkout_cap_failure = _unique_parent_path_accounting(
            ...
            projection_inputs={
                **projector_fixed_inputs,
                "checkout_cap": prior_parent_projection["checkout_accounting_bound"],
            },
        )
        self.assertEqual("blocked-worktree-capacity", checkout_cap_failure["status"])
        ...
        self.assertEqual("blocked-worktree-capacity", shared_headroom_failure["status"])
        ...
        self.assertEqual(
            1024 * 1024 * 1024 + checkout_parent_allocation_bound + targeted_bound,
        ...
        self.assertEqual("fail-closed-cross-filesystem-mismatch", mismatch["status"])
        ...
        self.assertEqual("fail-closed-overflow", grouped_projection_overflow["status"])
    ...
    def test_targeted_cleanup_resume_requires_live_custody_or_strict_backend(
        ...
        after_git_leaf_deleted = {
            path: identity for path, identity in manifest.items() if path != b".git"
        }
        live_kwargs = {
    ...
    def test_independent_codex_trust_profile_is_lightweight(self) -> None:
        ...
        self.assertIn(
            "difference between requested and observed runtime values", readiness
        )
    ...
    def test_independent_diff_is_sealed_into_raw_checkout(self) -> None:
        ...
        self.assertIn(
            "SHA-256 of the current durable control-artifact state", contracts
        )
        self.assertIn(
            "do not represent that control-state digest as part of the preflight attestation",
            contracts,
        )
        self.assertIn("including the 128 MiB maximum", contracts)
        self.assertIn("destination `.codex-review/review.diff`", contracts)
        self.assertIn(
            "helper-owned synthetic control artifact rather than a PR change", contracts
        )
        ...
        self.assertIn(
            "must not synchronously hash the potentially 128 MiB primary diff",
            contracts,
        )
        ...
        self.assertIn(
            "attempted concurrent cleanup instead waits or times out", readiness
        )
        self.assertIn("Do not fully read or hash", readiness)
        self.assertIn("not an unbounded pre-admission read", contracts)
        self.assertIn(
            "potentially 128 MiB streaming digest/copy only inside", contracts
        )
    ...
    def test_independent_diff_charge_is_in_checkout_ledger(self) -> None:
        ...
        self.assertIn(
            "grouped checkout charges including `review_diff_bound`", readiness
        )
        ...
        self.assertIn(
            "reservation, settlement, and retained-worktree charge", contracts
        )
    ...
    def test_independent_control_namespace_preserves_no_extra_integrity(self) -> None:
        ...
        self.assertIn(
            "`.codex-review/review.diff` placeholder with mode `0600`", readiness
        )
        self.assertIn(
            "Do not use `cp`, hard links, reflinks, symlinks, or path replacement",
            readiness,
        )
        self.assertIn("`fsync`, `fstat`, close, reopen no-follow", readiness)
        self.assertIn(
            "bounded frozen-index `filter` / `working-tree-encoding` query", readiness
        )
        ...
        self.assertIn(
            "no tracked symlink graph may resolve into `.codex-review`", contracts
        )
        self.assertIn(
            "one real `.codex-review` directory created mode `0700`", contracts
        )
        self.assertIn(
            "one empty `review.diff` regular placeholder created mode `0600`", contracts
        )
    ...
    def test_final_artifact_is_durable_before_terminal_review_state(self) -> None:
        ...
        irreversible_eof = contracts.index("irreversible transport boundary", transfer)
        eof = contracts.index(
            "publishes nothing until true reader EOF", irreversible_eof
        )
        ...
        self.assertIn(
            "independently repeats the no-follow identity/length/digest/exact-byte readback",
            contracts,
        )
        self.assertIn("final device/inode/length/SHA-256 seal metadata", contracts)
        self.assertIn(
            "strictly precede a trusted evidence-bearing terminal tuple", contracts
        )
        self.assertIn(
            "artifact verification, authorization, tuple publication", contracts
        )
        self.assertIn(
            "Controlled reader cancellation after Codex is proved never to have begun",
            contracts,
        ...
        self.assertIn(
            "`prelaunch-aborted` path with `review_status=not-run`", contracts
        )
    ...
    def test_attempt_supervisor_closes_reader_and_reviewer_after_outer_death(
        self,
    ) -> None:
        readiness = (SKILL_ROOT / "references/pr-readiness.md").read_text(
            encoding="utf-8"
        ...
        self.assertIn(
            "sole supervisor-owned peer is inherited by no other process", contracts
        )
        self.assertIn(
            "Close the reader peer only after the reader is reaped", contracts
        )
        ...
        self.assertNotIn(
            "parent closes its matching control endpoint on every handoff outcome",
            readiness,
        )
        ...
        completion_record = contracts.index(
            "returns a completion record", complete_handoff
        )
        ...
        settlement = contracts.index(
            "and settles instead of abandoning", reader_cancellation
        )
        ...
        self.assertIn(
            "any latched EOF immediately enters the complete-handoff abandonment path",
            contracts,
        )
        self.assertIn(
        ...
        self.assertIn(
            "EOF after the successful live-peer observation does not revoke", contracts
        )
    ...
    def test_process_and_checkout_share_physical_free_space_projection(self) -> None:
        ...
        self.assertIn(
            "map the process envelope to the retention-root identity", readiness
        )
        self.assertIn("Charges from all three roles are combined", contracts)
        self.assertIn(
        ...
        self.assertIn(
            "retention-root, checkout-root, and common-Git-directory filesystem identities",
            contracts,
        )
        ...
        process_shortfall = contracts.index(
            "proved shortfall as `blocked-retention`", process_projection
        )
    ...
    def test_raw_materializer_prevalidates_and_atomically_installs_symlinks(
        ...
        self.assertIn(
            "staging name to have the original placeholder identity", contracts
        )
