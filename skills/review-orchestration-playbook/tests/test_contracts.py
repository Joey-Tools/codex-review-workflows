...
class RepositoryContractTest(unittest.TestCase):
    ...
    def test_independent_codex_process_output_is_task_scoped_and_bounded(self) -> None:
        ...
        self.assertIn("deadline expires or either process-log limit", readiness)
        self.assertIn("limit-terminated attempt is inconclusive", readiness)
        self.assertIn("dedicated process group or session", readiness)
        self.assertIn("hard file limits or bounded sinks", readiness)
        self.assertIn("OS-enforced job/cgroup/container", readiness)
        self.assertIn("survives `setsid` / `setpgid`", readiness)
        self.assertIn("PID plus stable start identity", readiness)
        self.assertIn("bare PGID is insufficient", readiness)
        self.assertIn("all tracked descendants", readiness)
        self.assertIn("tree is quiescent", readiness)
        self.assertIn("separate 10-second deadline", readiness)
        self.assertIn("stat both process logs again", readiness)
        self.assertIn("even after exit zero", readiness)
        self.assertIn("quiescence or sink closure cannot be confirmed", readiness)
        self.assertIn("check its byte count before reading it", readiness)
        self.assertIn("remove an oversized artifact without reading it", readiness)
        ...
        self.assertIn("64 KiB for the final-message file", contracts)
        self.assertIn("send `TERM`", contracts)
        self.assertIn("send `KILL`", contracts)
        self.assertIn("when the deadline expires", contracts)
        self.assertIn("dedicated process group or session", contracts)
        self.assertIn("hard per-file limit or a bounded sink", contracts)
        self.assertIn("polling alone is not enforcement", contracts)
        self.assertIn("OS-enforced job, cgroup, or container", contracts)
        self.assertIn("survives `setsid` / `setpgid`", contracts)
        self.assertIn("PID and stable process-start identity", contracts)
        self.assertIn("never treat a bare PGID", contracts)
        self.assertIn("all tracked descendants", contracts)
        self.assertIn("repeat discovery and termination", contracts)
        self.assertIn("separate 10-second close deadline", contracts)
        self.assertIn("waiting indefinitely", contracts)
        self.assertIn("every tracked descendant identity is absent", contracts)
        ...
        self.assertIn("Check its byte count before reading it", contracts)
        self.assertIn("stat both process logs again", contracts)
        self.assertIn("even when it exited zero", contracts)
        self.assertIn("reaches or exceeds", contracts)
        self.assertIn("do not read it", contracts)
        self.assertIn("record only its byte count", contracts)
