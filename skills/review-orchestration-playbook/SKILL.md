...
The explicit phrases `double review`, `双重 review`, `triple review`, and `三重 review` are contemporaneous user consent for scoped code-review egress to OpenAI, Anthropic, and Microsoft/GitHub. The selected reviewer is a trusted processor inside this workflow's trust boundary. Consent covers the frozen tracked diff, necessary tracked context, and explicitly supplied review prompt in their original form, including repository secrets that pass the tracked-tree reduction gate; the helper does not rewrite these reviewer inputs to conceal those values. The prompt remains size-, identity-, and integrity-bound but is not a secret-egress filter. Triple review additionally authorizes current-head GitHub Codex review. Consent does not cover reviewer/runtime authentication credentials, automatically discovered untracked files, unrelated repositories, broad workspace dumps, or home-directory content. Read [egress-consent.md](references/egress-consent.md) before starting those lanes.
