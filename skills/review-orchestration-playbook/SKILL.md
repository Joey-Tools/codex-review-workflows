...
- Bind the accepted result to the current PR head and the current request: its trusted server timestamp must be strictly later than the exact request comment's API `created_at`. Same-head evidence from an earlier request is stale. A qualifying check must be terminal `completed` / `success` with non-null `completed_at` later than the request. Any code change invalidates earlier GitHub Codex evidence and requires a new request/result.
...
1. Classify the request.
   ...
   - Resolve a standalone named review deterministically: preserve an explicit frozen range first; otherwise use an explicitly named PR; otherwise accept only exactly one open PR associated with the exact current head repository/branch. An explicit-range-only single/double is fully scoped locally and requires no PR probe. More than one required PR candidate is `blocked-input` (the required explicit PR/range/target selector is absent), and no lane whose scope depends on that selection starts. An authenticated complete zero-candidate lookup proves the no-PR path, but does not supply a local review range: require an explicit committed range or explicitly named target/base from which to freeze `<merge_base>..HEAD`; never guess the target/base. For triple, detached HEAD or unknown head ownership without an explicit PR cannot prove no PR: run the scoped local lanes, but report the GitHub lane `blocked-input` and `effective: triple-inconclusive`, not effective double.
