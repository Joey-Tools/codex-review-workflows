...
- The existing supplied-diff/private-minimal-Git Codex helper is not this lane and does not satisfy single review.
...
- Follow **Canonical Executable Provenance** and the authentication/native-sandbox contracts in [canonical-claude-lane.md](references/canonical-claude-lane.md). [claude-runtime-trust.md](references/claude-runtime-trust.md) supplies shared signed-manifest verification primitives, version bounds, failure vocabulary, and the separate low-level helper contract only; the helper's verified executable snapshot, supplied-diff/private-minimal-Git workspace, explicit-auth precedence, clean/WIP capture, and post-run validation do not apply to this direct real-`HOME` lane.
...
The `isolated_review` helper retains a frozen supplied-diff runtime with a helper-private minimal Git view for low-level compatibility, diagnostics, and Claude runtime-security work. Its workspace and prompt contract differ from the canonical named shapes above, and its state records `review_contract=supplied-diff-private-git` and `named_lane_eligible=false`.
...
- Default to a clean exact-head detached workspace. `--include-source-wip` requires separate explicit consent, captures staged, unstaged, and non-ignored untracked content into a digest-bound review-only snapshot, and never supplies PR-readiness evidence.
- The low-level Claude path preserves the current user's real `HOME` as the ordinary CLI control plane and selects authentication in `ANTHROPIC_API_KEY` > `CLAUDE_CODE_OAUTH_TOKEN` > local-login order. It opaque-forwards only the winning explicit value and does not persist it in helper state.
- Treat its native sandbox as selected-deny enforcement plus prompt/model scope: requested global `denyWrite` and sensitive-root `denyRead` controls do not make `allowRead` a global host-read whitelist, and Claude Code 2.1.212 init output does not attest the merged sandbox.
- Accept only `explicit-claude-review` or `explicit-claude-with-copilot-fallback` as helper egress markers. The latter requires separate explicit consent for the supplemental Copilot diagnostic, which still never satisfies a named lane.
