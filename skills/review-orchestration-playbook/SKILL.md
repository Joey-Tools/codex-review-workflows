...
- Only complete structured terminal output collected after successful TERM/KILL/drain/reap cleanup of the initial supervisor process group and inherited streams may count. Timeout, either-stream overflow, drain/reap failure, residual members of that group, inherited-stream leaks, and malformed/partial terminal output are `inconclusive`; never accept a partial tail or fall back to another provider/model. This process-only supervisor is not a process-tree sandbox: a descendant that deliberately uses `setsid()` or `setpgid()` and closes inherited streams is outside its cleanup guarantee, so do not claim whole-process-tree quiescence.

The shipped guard supplies only the symlink-containment preflight and direct-Claude process supervision. It does not prepare a diff, perform review logic, establish executable provenance, configure or attest the sandbox, authenticate Claude, scan general content/secrets, or confer any `isolated_review` helper guarantee.
