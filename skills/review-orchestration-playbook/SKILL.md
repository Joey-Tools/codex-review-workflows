...
- Claude Code: when native Mach-O Claude Code and `rg` executables plus macOS `sandbox-exec` are available for verified safe-mode probes and final process/network isolation, use the ordinary local Claude login by default with `claude-opus-4-8` and `max`; local-login mode additionally requires `/usr/bin/clang` to build its restricted native Keychain broker. An explicitly exported `ANTHROPIC_API_KEY` remains an optional authentication override and does not require the broker. Fall back to `claude-opus-4-7` with `max` only after an explicit account, plan, organization-policy, or model-entitlement denial for Opus 4.8. Script/wrapper installations and hosts without the required secure runtime are treated as unavailable.
