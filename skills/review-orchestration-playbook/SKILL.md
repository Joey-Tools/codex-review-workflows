...
The explicit phrases `double review`, `双重 review`, `triple review`, and `三重 review` are contemporaneous user consent for scoped code-review egress to OpenAI, Anthropic, and Microsoft/GitHub. The selected reviewer is a trusted processor inside this workflow's trust boundary. Consent covers the frozen tracked diff and necessary tracked context in their original form, including repository secrets that pass the reduction gate; the helper does not rewrite tracked review evidence to conceal those values. The rendered review prompt is a separate caller-controlled surface: it is integrity-bound and secret-scanned, accepts only exact authoring-catalog values, and never inherits selected-legacy or dynamic-reduction eligibility. Triple review additionally authorizes current-head GitHub Codex review. Consent does not cover reviewer/runtime authentication credentials, untracked files, unrelated repositories, broad workspace dumps, or home-directory content. Read [egress-consent.md](references/egress-consent.md) before starting those lanes.
...
- When source or tests need credential-shaped fixtures, use `$synthetic-token-fixtures` to select an exact authoring token from the helper-owned catalog. Canonical tokens suppress only their declared scanner rule; credential-like paths at head and every other rule remain blocking.
...
- Before any Codex or Claude-family egress, require the helper's escaping-symlink and secret-delta preflight to pass. For each unregistered dynamic secret candidate, bounded exact counting across the complete base and head trees must prove `head_raw_count < base_raw_count` and `head_unembedded_count <= base_unembedded_count`. Equal residual counts, a new candidate, moves/copies, any count growth, or a candidate that cannot be extracted and counted safely under the budgets blocks review. A sensitive path deleted by the range may pass only when the complete head has no sensitive path; any sensitive path at head blocks. The helper-owned authoring and legacy catalog rules remain separate and unchanged. The PR/master gate is the primary defense against secret introduction; this gate permits review of a remediation only when the frozen range verifiably reduces exposure.
...
- Do not treat an unregistered secret's unchanged count as remediation. Dynamic candidates require a strict complete-tree raw-count reduction even when content merely moves or a path is renamed; unsafe or uncountable candidates block rather than falling back to reviewer-input rewriting.
