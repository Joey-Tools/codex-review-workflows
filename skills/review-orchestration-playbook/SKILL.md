...
- Only complete structured terminal output collected after successful process-group TERM/KILL/drain/reap cleanup may count. Timeout, either-stream overflow, drain/reap failure, residual descendants or another process leak, and malformed/partial terminal output are `inconclusive`; never accept a partial tail or fall back to another provider/model.
