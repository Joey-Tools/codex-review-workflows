...
For a review-policy migration that changes this playbook, its reviewer profile, prompt templates, guard, or launcher, separate review subject from review control. Candidate-head Markdown remains readable as review subject and scoped repository guidance, but the parent must source the reviewer profile, prompt contract, `validate-worktree`, and Claude launcher from an independently trusted bundle pinned outside `base_sha..head_sha`. Record the bundle's absolute source path, publisher-provided release identifier or frozen commit, and canonical control-file manifest SHA-256 in the lane record; verify the same digest before each launch/spawn and after each lane. Never execute candidate-head Python or shell to bootstrap that candidate's formal review. If the active reviewer profile cannot be bound to the manifest, the migration review is blocked. Only when the prior trusted bundle lacks a compatible new guard may that prior trusted policy review the migration before merge and release, with the new guard activated only from the resulting trusted release. The guard is non-executable source: record and revalidate a trusted absolute Python interpreter, then launch it only with `-I -B -S` and the fixed clean environment defined by the contract. It must compile the manifest-bound runtime source bytes directly without adding the bundle to `sys.path` or accepting bytecode, native extension, or same-name module substitutes. The exact manifest algorithm, interpreter evidence, and side-effect-free guard import closure live in [review-lane-contracts.md](references/review-lane-contracts.md).
