...
- After successful `validate-worktree`, launch the exact revalidated actual `claude` executable through the shipped `scripts/named_lane_guard run-claude` process-only supervisor from the clean worktree. Claude must be the direct child `argv[0]`, with direct argv/no shell, a bounded control prompt, a 1,800-second monotonic deadline, and 64 MiB each for stdout and stderr (128 MiB aggregate). For each private output path, the caller supplies a canonical real parent directory outside the worktree and an absent, non-symlink leaf. The launch environment is rebuilt from an explicit allowlist: `HOME`, `USER`, `LOGNAME`, and `SHELL` come from the current account's `pwd` record; `PATH` is trusted; only allowlisted locale/UI, proxy, and CA variables may pass through; Git is forced to use no lazy fetch, prompt, replacement objects, global/system config, or optional locks. Ambient Claude/Anthropic, cloud-provider, dynamic-loader, and tool-control variables are not inherited. This remains the canonical direct actual-Claude process—not `isolated_review` or a helper reviewer. Follow [canonical-claude-lane.md](references/canonical-claude-lane.md) for the full launch, process-group cleanup, and terminal-evidence contract.
