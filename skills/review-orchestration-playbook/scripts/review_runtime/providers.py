...
from .claude_keychain_macos import (
    KeychainIdentity,
...
CLAUDE_AUTH_LOGIN_ACTION = (
    "Run `claude auth login` or set `CLAUDE_CODE_OAUTH_TOKEN`, then restart "
    "the stateful review."
)
CLAUDE_OAUTH_TOKEN_ACTION = (
    "Unset or replace `CLAUDE_CODE_OAUTH_TOKEN`, then restart the stateful review."
)
CLAUDE_API_KEY_ACTION = (
    "`ANTHROPIC_API_KEY` takes precedence over `CLAUDE_CODE_OAUTH_TOKEN`; "
    "Unset or replace `ANTHROPIC_API_KEY`, then restart the stateful review."
)
...
_PROVIDER_OWNED_ATOMIC_TEXT_ARTIFACT_NAMES = frozenset(
    {
        "attempts.json",
        "claude-runtime.json",
        "claude-skip.txt",
...
class ClaudeCredentialWriteOutcomeUnknown(ClaudeCredentialInspectionInconclusive):
    """A supervised native Keychain replacement may have completed."""


class ClaudeCredentialWorkerTerminationInconclusive(
    ClaudeCredentialWriteOutcomeUnknown
):
    """A native Keychain replacement worker could not be proven reaped."""
...
def _claude_control_flow_from_cause_chain(
    ...
    for _ in range(16):
        if current is None or id(current) in seen:
            return None
        seen.add(id(current))
        if _is_claude_control_flow_error(current):
            return current
...
def _prepare_claude_keychain_broker(
    ...
    result = _claude_environment_with_selected_explicit_auth(env)
    if _claude_has_explicit_auth(result):
        return result
...
class _ClaudeMacOSCarrierSnapshot:
    ...
    keychain_identity: KeychainIdentity | None = None
...
def _read_claude_keychain_credential_snapshot(
    review: ReviewWorkspace,
) -> tuple[bytearray, KeychainIdentity] | None:
    del review
    try:
        return read_login_keychain_credential(
            _claude_keychain_account(),
            CLAUDE_KEYCHAIN_SERVICE,
        )
    except MacOSKeychainUnavailable as error:
        raise ClaudeKeychainBrokerUnavailable(str(error)) from error
    except MacOSKeychainUnsafe as error:
        raise ClaudeCredentialUnsafe(str(error)) from error
    except MacOSKeychainInspectionInconclusive as error:
        raise ClaudeCredentialInspectionInconclusive(str(error)) from error


def _read_claude_keychain_credential(
    review: ReviewWorkspace,
) -> bytearray | None:
    """Compatibility adapter for callers that do not retain writeback identity."""

    result = _read_claude_keychain_credential_snapshot(review)
...
def _claude_macos_carriers_match(
    ...
    try:
        keychain_result = _read_claude_keychain_credential_snapshot(review)
        if keychain_result is not None:
            keychain_credential, keychain_identity = keychain_result
        ...
        return (
            ...
            and keychain_identity == expected.keychain_identity
...
def _read_claude_macos_carrier_snapshot(
    ...
    try:
        ...
        return _ClaudeMacOSCarrierSnapshot(
            ...
            keychain_identity=keychain_identity,
...
def _write_claude_keychain_credential(
    ...
    *,
    ...
    try:
        ...
        return False
    expected_identity = carrier_snapshot.keychain_identity
    ...
    try:
        ...
        with update_lock_context:
            ...
            with refresh_lock_context as refresh_lock:
                ...
                if not carriers_already_matched and not _claude_macos_carriers_match(
                    review,
                    carrier_snapshot,
                ):
                    return False
                current_result = _read_claude_keychain_credential_snapshot(review)
                if current_result is None:
                    return False
                current, current_identity = current_result
                try:
                    if current_identity != expected_identity or not (
                        hmac.compare_digest(current, expected_credential)
                    ):
                        return False
                    refresh_lock.assert_held()
                    try:
                        updated_identity = replace_login_keychain_credential(
                            _claude_keychain_account(),
                            CLAUDE_KEYCHAIN_SERVICE,
                            current,
                            credential,
                        ...
                        raise ClaudeCredentialWorkerTerminationInconclusive(
                            str(error)
                        ) from error
                    except MacOSKeychainWriteOutcomeUnknown as error:
                        raise ClaudeCredentialWriteOutcomeUnknown(str(error)) from error
                    except (
                        MacOSKeychainUnavailable,
                        MacOSKeychainInspectionInconclusive,
                    ) as error:
                    ...
                    refresh_lock.assert_held()
                    return updated_identity == expected_identity
                finally:
...
def _persist_claude_macos_refreshed_credential_impl(
    ...
    with _claude_macos_carrier_coordination(
        ...
        current_keychain_identity: KeychainIdentity | None = None
        current_file: bytearray | None = None
        try:
            current_keychain_result = _read_claude_keychain_credential_snapshot(review)
            if current_keychain_result is not None:
                current_keychain, current_keychain_identity = current_keychain_result
            ...
            if not (
                ...
                and current_keychain_identity == carrier_snapshot.keychain_identity
                and _claude_optional_credential_digest_matches(
                    current_file,
                    file_digest,
                )
            ...
            if write_keychain:
                ...
                for attempt_index in range(CLAUDE_MACOS_DUAL_CARRIER_KEYCHAIN_ATTEMPTS):
                    ...
                    try:
                        ...
                        deferred_control_flow = (
                            _claude_control_flow_from_cause_chain(
                                keychain_write_error
                            )
                        )
                        worker_termination_inconclusive = isinstance(
                            keychain_write_error,
                            ClaudeCredentialWorkerTerminationInconclusive,
                        )
                        if worker_termination_inconclusive:
                            refresh_lock.retain_for_controlled_cleanup(
                    ...
                    if (
                        ...
                        deferred_control_flow = ForwardedSignal(
                            pending_attempt_signal
                        )
                    if worker_termination_inconclusive:
                        detail = (
                            "the native Keychain replacement worker could not be "
                            "proven terminated; the shared writer lock was retained "
                        ...
                        raise ClaudeCredentialInspectionInconclusive(
                            "the native Keychain replacement worker could not be "
                            "proven terminated, so its write outcome cannot be "
                            "reconciled or retried safely"
                        ) from keychain_write_error
                    if keychain_written:
                        if deferred_control_flow is not None:
                            raise deferred_control_flow
                    ...
                    try:
                        ...
                        if deferred_control_flow is None and (
                        ...
                        detail = (
                            ...
                            "its single bounded readback was inconclusive; the "
                            "replacement was not retried"
                        )
                        if deferred_control_flow is not None:
                            _attach_claude_persistence_signal_detail(
                                deferred_control_flow,
                        ...
                        raise ClaudeCredentialInspectionInconclusive(
                            "the native Keychain replacement outcome is unknown and "
                            "its single bounded readback was inconclusive; review is "
                            "paused without retrying the replacement"
                        ) from error
                    refresh_lock.assert_held()
                    keychain_is_refreshed = (
                        hmac.compare_digest(
                        ...
                        and readback.keychain_digest is not None
                        and readback.keychain_identity
                        == carrier_snapshot.keychain_identity
                    )
                    expected_file_digest = (
                        refreshed_digest if write_file else file_digest
                    )
                    ...
                    if keychain_is_refreshed and file_is_expected:
                        if deferred_control_flow is not None:
                            _attach_claude_persistence_signal_detail(
                                deferred_control_flow,
                                "the native Keychain replacement was reconciled as "
                                "completed before restoring the interrupted control "
                                "flow",
                            )
                            raise deferred_control_flow
                        return readback
                    ...
                    if not (keychain_is_original and file_is_expected):
                        if deferred_control_flow is not None:
                            _attach_claude_persistence_signal_detail(
                                deferred_control_flow,
                                "Claude credential carriers changed unexpectedly "
                                "during the single bounded reconciliation readback",
                            )
                            raise deferred_control_flow
                        raise ClaudeCredentialInspectionInconclusive(
                    ...
                    if deferred_control_flow is not None:
                        _attach_claude_persistence_signal_detail(
                            deferred_control_flow,
                            "the single bounded Keychain reconciliation readback "
                            "showed the original carrier; the replacement was not "
            ...
            if not (
                ...
                and observed.keychain_identity == carrier_snapshot.keychain_identity
                and hmac.compare_digest(
...
def _claude_explicit_auth_source(env: dict[str, str]) -> str | None:
...
def _claude_explicit_auth_environment(env: dict[str, str]) -> dict[str, str]:
    source = _claude_explicit_auth_source(env)
    if source == "api-key":
...
def _claude_environment_with_selected_explicit_auth(
    env: dict[str, str],
) -> dict[str, str]:
    result = dict(env)
    for key in CLAUDE_EXPLICIT_AUTH_ENV_KEYS:
        result.pop(key, None)
    result.update(_claude_explicit_auth_environment(env))
    return result


def _claude_has_explicit_auth(env: dict[str, str]) -> bool:
    return _claude_explicit_auth_source(env) is not None


def _claude_auth_action(env: dict[str, str]) -> str:
    source = _claude_explicit_auth_source(env)
    if source == "api-key":
...
def _claude_explicit_auth_secrets(
    env: dict[str, str],
) -> tuple[str, ...]:
    """Freeze every configured explicit secret before winner selection."""
...
def _claude_explicit_auth_redact_values(
    env: dict[str, str],
) -> tuple[str, ...]:
    return output_redact_values(_claude_explicit_auth_secrets(env))


def _provider_atomic_write_filter(
    review: ReviewWorkspace,
) -> Callable[[pathlib.Path], bool]:
    root = review.container_dir.expanduser().absolute()
    attempts_root = root / "attempts"
    protected_inputs = frozenset(
        {
            review.diff_file.expanduser().absolute(),
            review.prompt_file.expanduser().absolute(),
        }
    )
    workspace_root = review.workspace_root.expanduser().absolute()

    def includes(path: pathlib.Path) -> bool:
        candidate = path.expanduser().absolute()
        if (
            candidate in protected_inputs
            or candidate == workspace_root
            or is_relative_to(candidate, workspace_root)
        ):
            return False
        return (
            candidate.parent == root
            and candidate.name in _PROVIDER_OWNED_ATOMIC_TEXT_ARTIFACT_NAMES
        ) or candidate.parent == attempts_root

    return includes


def _redact_claude_bytes(
    payload: bytes,
    redact_secrets: Iterable[str],
) -> bytes:
    secrets = tuple(redact_secrets)
    if not payload or not secrets:
        return payload
    return os.fsencode(redact_text(os.fsdecode(payload), secrets))


def _redact_claude_text(
    text: str,
    redact_secrets: Iterable[str],
) -> str:
    return redact_text(text, redact_secrets)


def _redact_claude_completed(
    completed: Completed,
    redact_secrets: Iterable[str],
) -> Completed:
    frozen = tuple(redact_secrets)
    if not frozen:
        return completed
    return Completed(
        argv=tuple(_redact_claude_text(item, frozen) for item in completed.argv),
        returncode=completed.returncode,
        stdout=_redact_claude_bytes(completed.stdout, frozen),
        stderr=_redact_claude_bytes(completed.stderr, frozen),
    )
...
def _with_claude_review_tool_path(
    ...
    if not _is_claude_linux_host() and not _claude_has_explicit_auth(env):
        broker_dir = (
...
def _claude_review_sandbox_profile(
    ...
    *,
    ...
    if not _claude_has_explicit_auth(env):
...
def _parse_claude_output(
    ...
    *,
    ...
    stdout = _redact_claude_bytes(stdout, redact_values)
...
def _record_attempt(
    *,
    ...
    frozen_redact_values = tuple(redact_values)
    completed = _redact_claude_completed(completed, frozen_redact_values)
    if final_text is not None:
        final_text = _redact_claude_text(final_text, frozen_redact_values)
    if effective_model is not None:
        effective_model = _redact_claude_text(
            effective_model,
            frozen_redact_values,
        )
...
def _resolve_validated_claude_executable(
    *,
    ...
    prepared_env = _claude_environment_with_selected_explicit_auth(env)
    ...
    def validate_candidate(candidate: pathlib.Path) -> None:
        ...
        if isinstance(verified, VerifiedClaudeExecutable):
            ...
            runtime_reports[str(candidate.absolute())] = {
                ...
                "authentication": {
                    "source": _claude_explicit_auth_source(prepared_env) or "pending",
                    "carrier": (
                        "environment"
                        if _claude_has_explicit_auth(prepared_env)
                        else (
...
def _claude_linux_review_runtime(
    ...
    with contextlib.ExitStack() as stack:
        auth_env = _claude_explicit_auth_environment(env)
        explicit_auth_source = _claude_explicit_auth_source(env)
        if auth_env:
        ...
        _update_claude_runtime_report(
            review,
            {
                ...
                "authentication": {
                    "source": explicit_auth_source or "credential-file",
                    "carrier": (
                        "environment"
                        if explicit_auth_source is not None
...
def _claude_attempt(
    *,
    ...
    frozen_redact_values = (
        _claude_explicit_auth_secrets(env)
        if redact_values is None
        else tuple(redact_values)
    )
    env = _claude_environment_with_selected_explicit_auth(env)
    if executable is None:
    ...
    if _claude_has_explicit_auth(env):
    ...
    if not linux_host:
        _update_claude_runtime_report(
            review,
            {
                ...
                "authentication": {
                    "source": _claude_explicit_auth_source(env) or "pending",
                    "carrier": (
                        "environment"
                        if _claude_has_explicit_auth(env)
                        else "one-shot-security-broker"
    ...
    if linux_host:
        ...
        try:
            with _claude_linux_review_runtime(
                ...
                completed = _redact_claude_completed(
                    run(
                        ...
                        redact_values=output_redact_values(frozen_redact_values),
                    ),
                    frozen_redact_values,
                )
    ...
    final_text, effective_model = _parse_claude_output(
        completed.stdout,
        requested_model=model,
        redact_values=frozen_redact_values,
    )
    attempt = _record_attempt(
        review=review,
        index=index,
...
def _copilot_attempt(
    *,
    ...
    frozen_redact_values = tuple(redact_values)
    output_redactions = output_redact_values(frozen_redact_values)
    ...
    permission_help = _redact_claude_completed(
        run(
            ...
            redact_values=output_redactions,
        ),
        frozen_redact_values,
    )
...
def _finish(
    ...
    *,
    ...
    for index, attempt in enumerate(attempts):
        redacted_final = (
            _redact_claude_text(attempt.final_text, frozen_redact_values)
            if attempt.final_text is not None
            else None
        )
        redacted_effective_model = (
            _redact_claude_text(attempt.effective_model, frozen_redact_values)
            if attempt.effective_model is not None
            else None
        )
        if (
            redacted_final != attempt.final_text
            or redacted_effective_model != attempt.effective_model
        ):
            attempts[index] = replace(
                attempt,
                final_text=redacted_final,
                effective_model=redacted_effective_model,
            )
    if final_text is not None:
        final_text = _redact_claude_text(final_text, frozen_redact_values)
...
def _finish_claude_auth_required(
    ...
    *,
    ...
    frozen_redact_values = tuple(redact_values)
    detail = _redact_claude_text(detail, frozen_redact_values)
    action = _redact_claude_text(action, frozen_redact_values)
    for index, attempt in enumerate(attempts):
        if attempt.final_text is not None:
            attempts[index] = replace(
                attempt,
                final_text=_redact_claude_text(
                    attempt.final_text,
                    frozen_redact_values,
                ),
            )
    ...
    *,
    ...
    for model in models:
        ...
        try:
            ...
            stdout_path, stderr_path = _attempt_paths(review, index, runtime, model)
            stdout_path.touch(exist_ok=True)
            diagnostic = _redact_claude_text(
                f"review supervision failed: {error}",
                frozen_redact_values,
            )
...
def _run_review_with_frozen_auth(
    *,
    review: ReviewWorkspace,
    reviewer: str,
    ...
    if reviewer == "claude":
        if claude_env is None:
            raise ReviewError("the frozen Claude environment is unavailable")
    ...
    try:
        ...
        write_text_atomic(
            review.container_dir / "runner-error.txt",
            "review egress workspace preflight failed: "
            f"{_redact_claude_text(str(error), claude_redact_values)}\n",
        )
        return Outcome(2, None, tuple())
    ...
    try:
        ...
        return _finish_claude_auth_required(
            review,
            attempts,
            _redact_claude_text(str(error), claude_redact_values),
    ...
    if claude_available and claude_executable is not None:
        ...
        try:
            ...
            detail = _redact_claude_text(str(error), claude_redact_values)
            if persistence_diagnostic is not None:
        ...
        if category == "auth":
            return _finish_claude_auth_required(
                ...
                attempts,
                "the restricted Claude runtime rejected the configured credential",
                action=_claude_auth_action(claude_env),
                redact_values=claude_redact_values,
            )
    ...
    try:
        ...
        write_text_atomic(
            review.container_dir / "runner-error.txt",
            "Copilot CLI executable validation failed: "
            f"{_redact_claude_text(str(error), claude_redact_values)}\n",
        )
        _write_attempts(review, attempts)
        return Outcome(2, None, tuple(attempts))
    ...
    def run_copilot_attempt_with_startup_redactions(
        *,
        review: ReviewWorkspace,
        model: str,
        index: int,
    ...
    try:
        _, final_text = _run_model_chain(
            ...
            runner=run_copilot_attempt_with_startup_redactions,
            runtime="copilot",
            requested_effort=COPILOT_REASONING_EFFORT,
            env=copilot_env,
            attempts=attempts,
        ...
        write_text_atomic(
            review.container_dir / "runner-error.txt",
            "Copilot review was inconclusive: "
            f"{_redact_claude_text(str(error), claude_redact_values)}\n",
        )
        _write_attempts(review, attempts)
        return Outcome(75, None, tuple(attempts))
        ...
        write_text_atomic(
            review.container_dir / "runner-error.txt",
            "Copilot CLI became unavailable or failed executable validation: "
            f"{_redact_claude_text(str(error), claude_redact_values)}\n",
        )
        _write_attempts(review, attempts)
        return Outcome(2, None, tuple(attempts))
...
def run_review(
    *,
    ...
    if reviewer == "claude":
        ...
        claude_redact_values = _claude_explicit_auth_secrets(raw_claude_env)
        claude_env = _claude_environment_with_selected_explicit_auth(raw_claude_env)

    with atomic_write_redactions(
        claude_redact_values,
        path_filter=_provider_atomic_write_filter(review),
    ):
        return _run_review_with_frozen_auth(
            review=review,
            reviewer=reviewer,
