...
- 查询 PR metadata、review threads、rulesets、branch protection 或 merge state 时，先用稳定的 typed `gh` forms，例如 `gh pr view --json ...`、`gh pr checks`、`gh pr status`、`gh api repos/<owner>/<repo>/branches/<base>/protection`。只有 typed forms 不够时才写自定义 `gh api graphql`，且 query 必须是当前决策所需的最小字段集合。不要把含 `$owner`、braces、aliases、multiline selection 或长 field list 的 GraphQL 裸贴成 shell 里未引用的一行 `-f query=...`；把复杂 query 放进 task-scoped `.codex-tmp/.../*.graphql` 或短脚本变量，再用 quoting-safe invocation 传给 `gh api graphql`。带 `?` 的 REST endpoint 必须作为一个整体引用，例如 `gh api 'repos/<owner>/<repo>/rulesets?ref=refs/heads/<base>'`，否则 zsh 会把它当 glob。GraphQL `Field ... doesn't exist on type ...`、`Expected NAME` 这类 schema/parse error 是 probe failure；先删掉或验证出错字段，再重试最小查询，不要继续扩大同一个 query。
