...
When a triple-review request also includes a local double-review half that would use a non-Codex external reviewer, treat that reviewer as separately opt-in. Workflow-implied triple-review consent covers only the Codex/GitHub lanes described here; if separate non-Codex consent is absent, report that half as blocked instead of running it or silently downgrading the requested review shape.
