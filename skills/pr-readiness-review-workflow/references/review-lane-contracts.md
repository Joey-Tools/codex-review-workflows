...
在 review-only 子线程里，先说“只开窄窗口”不够；每次实际 tool call 前都要把命令形状和上面的禁令核对一遍。看到裸 `nl -ba <file>`、`cat <file>`、`git show <rev>:<path>`、multi-file/path-wide `rg -n`、single-file broad-pattern `rg -n`、宽 `git diff --unified=*` 或低上下文 multi-file selected diff 时，先改写成 count probe、exact symbol lookup、单 hunk 或窄 `sed -n '<start>,<end>p'` window，再执行。
