...
Tool-call self-check: Before every tool call, rewrite bare `nl -ba <file>`, `cat <file>`, `git show <rev>:<path>`, path-wide or multi-file `rg -n`, and wide selected-file diffs into count probes, exact symbol lookups, single-hunk reads, or narrow `sed -n '<start>,<end>p'` windows.
