...
- 如果 the user 要求 full workflow、merge-ready、`在合并前停止`、`stop before merge`，先记录需要 PR readiness handoff。该措辞只在 `$pr-readiness-review-workflow` 的 PR target authorization preflight 通过后，允许后续创建/更新 review-ready PR 和等待 review/CI；不允许 merge。
...
- 非 Codex external reviewers 不是默认门禁；只有 the user 明确要求或 `$pr-readiness-review-workflow` 选择 opt-in 时才运行。
