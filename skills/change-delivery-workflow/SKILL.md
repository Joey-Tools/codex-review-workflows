...
- Named triple review 必须再包含 exact host `github.com` PR 当前 head 上的 exact `@codex review` 请求，以及 exact REST `chatgpt-codex-connector[bot]` / `Bot` 身份提供的完整 current-scope provider evidence。请求历史只控制 producer 是否可以再次发请求；consumer outcome 按 `$review-orchestration-playbook` 的 `github-codex-evidence-authority.md` 读取完整 comments、reviews、inline comments、threads 与 reactions 后判定，不要求 request/run attribution。Early/duplicate 请求只产生 `request_policy` warning，parent 仍不得主动发第二或第三条。Exact App `chatgpt-codex-connector` 的 current-head post-request check/run 只能作为 service-start evidence；即使 `completed` / `success`，也不能完成 triple 或证明 clean/no-findings。无 PR、直接观察到任何 non-`github.com` host（包括 `sqbu-github.cisco.com` 与所有 GitHub Enterprise host），或 operating identity in `{hoteng, hoteng_cisco}` 时，明确报告 `effective double`。Integration/service unavailable 只能由 authority 定义 schema 下的 authenticated structured capability or installation metadata 证明，而且该 metadata 必须标识 selected repository/integration 并明确编码 unavailable 或 not-installed；不得以泛称 “unsupported”、缺失响应或 free-form provider prose 降级。已启动后，otherwise valid nonterminal/check-only evidence、missing terminal payload 或 retryable incomplete pagination remain `pending` while bounded waiting is meaningful；等待耗尽后才报告 `triple-inconclusive`。Malformed/stale evidence、ambiguous identity/scope、non-retryable failure 或 permanently incomplete enumeration 立即报告 `triple-inconclusive`；两类情况都不得降成 double。
