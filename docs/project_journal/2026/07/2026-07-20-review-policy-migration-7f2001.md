...
- The low-level supplied-diff helper is explicitly ineligible for named-lane counting. Its stateful surface records `review_contract: supplied-diff-private-git` and `named_lane_eligible: false`; raw foreground or `stateful final` output is never a named-lane envelope.
