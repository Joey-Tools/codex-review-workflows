...
- Final local review then caught an ordering edge case: no-PR and unsupported lanes must be classified unavailable before consulting PR-only head fields. Availability now precedes head comparison, which runs only for an existing supported third-lane candidate.
