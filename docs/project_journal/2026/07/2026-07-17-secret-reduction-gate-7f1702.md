...
- A complete generic literal RHS remains an exact dynamic candidate when balanced wrappers or triple quotes enclose it, including embedded newlines. This permits strict `1 -> 0` and `2 -> 1` remediation for those forms; unsupported, unclosed, oversized, backtick, or ambiguous forms remain blocking.
...
- Post-integration frozen-range review found and fixed two scope/scanner gaps: initialized submodule content can no longer enter `review.diff`, and wrapped/triple/multiline generic literal assignments now participate in exact reduction proof. `PublicPoolScannerTest`: 91 passed. Synthetic-token module: 206 passed.
...
- Python 3.13 full suite: 1200 tests passed, 4 skipped.
- Python 3.10.19 full suite: 1200 tests passed, 4 skipped.
