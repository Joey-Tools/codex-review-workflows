...
- The macOS Keychain broker explicitly tracks every handler, closes active sockets, and bounds serve-loop, handler, and recovery shutdown. Its server-owned pending slot preserves the newest complete rotation by generation. If handler quiescence cannot be proven, the runtime enters a terminal abandoned state before recovery, late handlers cannot commit state, and one shared recovery-candidate path is retained or reported for operator inspection. Review timeout and forwarded-signal control flow remain primary while carrying the persistence failure and recovery path; broker uncertainty pauses the lane and never authorizes Copilot fallback.
...
- The complete helper suite passed after review fixes and integration with the latest `master`: 836 tests run, 13 skipped in the sandboxed full run. Five focused Keychain broker tests also passed with real loopback access, covering blocked handler timeout/signal/recovery, latest-generation pending updates, serve-loop failure propagation, shared recovery-candidate ownership, and unquiescent-handler recovery.
