...
- Effective included `core.fsmonitor` is validated before any status or reviewer Git command. Unset and Git-false values are allowed; built-in daemon, no-value, and path hook values are rejected without execution. Only the final effective value is authoritative, so a later worktree or local `false` safely overrides an earlier included path without a false finding.
