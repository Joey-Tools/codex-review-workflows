...
- The same absolute monotonic deadline now begins before the CLI reads the control prompt through EOF and can only tighten the duration bound. A writer that sends a short prompt but withholds EOF becomes an inconclusive deadline result without launching the reviewer.
...
- The final loopback-enabled full review-policy suite passed all 1,114 tests with four skips in 226.399 seconds. During an earlier repair iteration, one unchanged concurrent-cleanup test hit a transient identity race; it passed five consecutive exact reruns and did not recur in any final full-suite run.
