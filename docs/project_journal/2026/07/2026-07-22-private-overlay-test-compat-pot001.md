...
- Build malicious bytecode fixtures at the cache location used by the isolated `-I` guard subprocess, even when the parent test process has `PYTHONPYCACHEPREFIX` set.
...
- Unchecked bytecode fixtures recursively create their isolated cache parents and still exercise the path the guarded interpreter would inspect.
...
- `test_named_lane.py`: 163 tests passed with `PYTHONPYCACHEPREFIX` set.
