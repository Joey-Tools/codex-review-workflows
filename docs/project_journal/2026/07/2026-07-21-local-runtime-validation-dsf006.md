...
- On the multi-version path, the first existing finite set source wins: an exact or uniquely resolvable task/user set, explicit repository policy, or a finite repository supported-version set/CI matrix. Lower-priority sets are not compared or merged; only conflicts inside the selected source block, along with ambiguous, open-ended, or incompatible sets.
